...
affiliation:
  - code: 1
    address: Pacific Biological Station, Fisheries and Oceans Canada, Nanaimo, BC, Canada
  - code: 2
    address: Northwest Fisheries Science Center, National Marine Fisheries Service, National Oceanic and Atmospheric Administration, Seattle, WA, USA
  - code: 3
    address: Alaska Fisheries Science Center, National Marine Fisheries Service, National Oceanic and Atmospheric Administration, Seattle, WA, USA
output:
    bookdown::pdf_book:
      template: template-peerj.tex
      pandoc_args: "--highlight=haddock"
      toc: false
      number_sections: false
      keep_tex: true
abstract: >
  Geostatistical data---spatially referenced observations of a continuous spatial phenomenon---are ubiquitous in ecology and can reveal ecological processes and inform management decisions.  However, appropriate models to analyse these data, such as generalised linear mixed effects models (GLMMs) with Gaussian random fields, are computationally intensive and challenging to implement.  Therefore, spatial autocorrelation is frequently ignored, or addressed in less ecologically interpretable ways. Here, we introduce the R package sdmTMB, which extends the flexible interface familiar to users of lme4, glmmTMB, or mgcv to include spatial and spatiotemporal latent fields using a predictive-process SPDE- (stochastic partial differential equation) based approach.  Estimation is conducted via maximum marginal likelihood with Template Model Builder (TMB), although penalised likelihood and Bayesian inference (via tmbstan/Stan) are also possible.  We describe the statistical model, illustrate the package's use through two case studies, and compare the functionality, speed, and interface to related software. The case studies illustrate sdmTMB’s unmatched flexibility and usability by implementing penalised smoothers, non-stationary processes (time-varying and spatially varying relationships), and anisotropy (asymmetry in spatial correlations that commonly occurs along ecological gradients and is not available in INLA/inlabru).  Modelling of these ecologically ubiquitous, but computationally complex, relationships is further aided by built-in handling of delta/hurdle models, break-point effects, abundance index standardisation, and diagnostic tools including out-of-sample cross validation.  Finally, we demonstrate that sdmTMB is 4–16 times faster than INLA or inlabru. sdmTMB's accessible interface has already opened this useful class of models to a wider audience within species distribution modelling.  We anticipate that its application to the full spectrum of problems involving geostatistical data will prove equally fruitful.
...
Packages such as VAST [@thorson2019] and inlabru [@bachl2019] are powerful user interfaces to fit spatial models that use the SPDE, but they either lack a modular interface familiar to those who have used lme4 or glmmTMB, or lack key functionality.
...
The Matérn can accommodate a range of shapes and can be both isotropic (covariance decays at the same rate in all directions) or anisotropic (covariance in the latitudinal direction may differ from the longitudinal direction) [@haskard2007].
...
See Appendix S1 for a full description of the model.
...
By default, if spatiotemporal fields are included, they are assumed IID; however, additional options allow them to be modelled as a random walk or first-order autoregressive, AR(1), process (Fig.\ \@ref(fig:diagram); Appendix S1).
Turning off both spatial and spatiotemporal effects allows comparison with a standard non-spatial GLM or GLMM.
We include additional flexibility in specifying the linear fixed effect matrix: covariates can be modelled as penalized smooth functions (generalized additive models, GAMs) using the same `s()` syntax as in mgcv [@wood2017a] (thereby allowing automatic selection of smoother 'wiggliness'), or can be modelled with threshold shapes; for example, hockey-stick models, `breakpt()`, [@barrowman2000] or logistic functions, `logistic()` (Appendix S1).
...
After rapid model exploration with maximum likelihood, one can optionally pass an sdmTMB model to the R package tmbstan [@monnahan2018] to estimate the joint posterior distribution with Stan [@carpenter2017] for Bayesian inference.

sdmTMB models can include penalized likelihoods by assigning priors to model parameters (`?sdmTMBpriors`).
These priors may be useful in cases where estimation is difficult because of identifiability issues or relatively flat likelihood surfaces, to impart prior information, to or achieve regularization.
...
# Example using fish survey data {-}
...
```{r pcod-fig, fig.cap="(ref:pcod-fig-cap)", out.width="5.2in", fig.align='center', fig.pos="htb"}
...
An alternative would be the `delta_gamma()` family to specify a delta/hurdle model [@aitchison1955] or to model catch weight with an offset for log(area swept).
We could allow for spatial anisotropy by adding `anisotropy = TRUE` [@lindgren2011; @thorson2015ices].
...
We could extend the spatial model to be a spatiotemporal one simply by supplying the year column name to the `time` argument and specifying how we want the spatiotemporal random fields to be structured.
...
We found a weak average positive effect of the annual mean NAO on overall counts, but a southeast to northwest gradient in the intensity of the effect (Fig. \@ref(fig:owl-nao), Appendix S3).
...
```{r owl-nao, fig.cap="(ref:owl-nao-fig)", out.width="4.1in", fig.align='center', fig.pos="hb"}
...
(3) An sdmTMB model can be passed to the tmbstan package [@monnahan2018] to sample from the joint posterior with Stan [@carpenter2017], evaluate the accuracy of the Laplace approximation, or perform posterior predictive checks (see `?extract_mcmc`).
...
There are several R packages capable of fitting geostatistical spatial or spatiotemporal models [e.g., @heaton2019].
...
sdmTMB and mgcv focus on univariate response data, whereas VAST, INLA/inlabru, spaMM, and spBayes extend to multivariate responses with various limitations.
...
We provide comparisons of the syntax and the reproducibility of results from models fit using INLA or inlabru (Appendix S4) and VAST (Appendix S5).
...
<!-- sdmTMB used the default settings and `sdmTMBcontrol(normalize = TRUE)`, which calculates the random effect normalizing constant in R. -->
...
Note that spaMM only fits spatial, not spatiotemporal, models.
...
```{r timing, fig.cap="(ref:timing-fig)", out.width="3.5in", fig.align='center', fig.pos="htb"}
f <- here::here("figs", "timing-spatial-m1-april11.pdf")
knitr::include_graphics(f)
```


```{r compare-table, results='asis'}
library(kableExtra)
...
knitr::kable(d, format = "latex", caption = "\\textbf{Comparison of functionality between several R packages that can fit geostatistical GLMMs.}",
  booktabs = TRUE,
  linesep = c(
    rep('', 4), # coefs
    '\\addlinespace',
...
We ran a simple speed comparison between sdmTMB, inlabru/INLA, and mgcv for fitting an SPDE spatial random field model to 1000 data points with Gaussian error across a range of mesh resolutions (Fig. \@ref(fig:timing), Appendix S6).
In this test, sdmTMB was the fastest across all mesh resolutions; however, inlabru/INLA and spaMM were less affected by mesh resolution than sdmTMB.
mgcv was most affected by mesh resolution.
...
Results with optimized math libraries on one core (openBLAS: @penblas2021 and PARDISO: @pardiso-7.2c) resulted in a ~10% speed increase for sdmTMB and inlabru and a ~7--9-fold speed increase for mgcv.
...
alternatively, because VAST focuses on multivariate delta models and fisheries applications (Appendix S5), users fitting "simple" univariate spatial/spatiotemporal GLMMs in non-fisheries contexts may find sdmTMB more straightforward to use.
...
<!-- Speed-wise, the TMB-based packages sdmTMB and VAST were fastest up to at least 1000 mesh nodes, which in practice can cover fairly complex spatial processes on large datasets [e.g., @anderson2019b; @maureaud2021; @english2022]. -->


Speed-wise, sdmTMB (and by association VAST) were fastest up to at least 1000 mesh nodes at approximately 4--16 times faster than INLA/inlabru across the range of mesh complexities considered here.
...
However, for users ultimately interested in Bayesian inference, the approximate Bayesian inference offered by INLA/inlabru is likely to be considerably faster than passing the same model from sdmTMB/VAST to tmbstan for full Bayesian inference.
...
sdmTMB would not be possible without the TMB [@kristensen2016] and INLA [@rue2009; @lindgren2011; @lindgren2015] R packages. sdmTMB is heavily inspired by and in some places code has been adapted from both the VAST [@thorson2019] and glmmTMB [@brooks2017] R packages. Penalized spline support was possible thanks to mgcv [@wood2017a]. We thank the authors of all these packages.

*Individual acknowledgements omitted from this submitted main text file*
...
Appendix S1: sdmTMB statistical model description\
Appendix S2: Example of index standardization of fishery-independent survey data with sdmTMB\
Appendix S3: Example of a spatially varying effect of climate on citizen science count data with sdmTMB\
Appendix S4: Species distribution model comparison between INLA and sdmTMB\
Appendix S5: Comparison of index standardization of survey data with VAST and sdmTMB\
Appendix S6: sdmTMB speed testing methods and model validation description
