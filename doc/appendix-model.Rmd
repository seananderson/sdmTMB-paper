...
symbols <- tibble::tribble(
  ...
  "$\\zeta_{\\s, t}$", "\\texttt{zeta\\_t}", "Spatially varying coefficient random field at point $\\s$ and time $t$ (knot)",
  "$\\zeta_{\\s, t}^*$", "\\texttt{zeta\\_t\\_A}", "Spatially varying coefficient random field at point $\\s$ and time $t$ (interpolated)",
  ...
  "$\\delta_{\\s,t}$", "\\texttt{b\\_t}", "AR1 or random walk spatiotemporal deviations (knot)",
...
<!-- should alpha_g be bold as in the equation above? --><!-- EW: yes -- alpha_g should be bolded here -->
...
### Smoothers
<!-- EW: see this paragraph and below, should package names be enclosed in single quotes, like `sdmTMB()` above? Examples: mgcv, brms, sdmTMB throughout -->
<!-- EW: unrelated, I never tried to implement factor smooths. I assume they work, but it'd good to check eventually -->
...
<!-- EW: edited omega in this section to be bolded and subscripted with s, for consistency with above --> 
Spatial random fields, $\omega_{\bm{s}}$, are included if `spatial = 'on'` and omitted if `spatial = 'off'`.

$$
\begin{aligned}
\mu_{\bm{s},t} &= f^{-1} \left( \ldots + \boldsymbol{\omega_s} + \ldots \right),\\
\boldsymbol{\omega_s} &\sim \operatorname{MVNormal} \left( \boldsymbol{0}, \boldsymbol{\Sigma}_\omega \right),\\ 
\end{aligned}
$$
...
$$
\boldsymbol{\omega_s} \sim \mathrm{MVNormal}\left(\boldsymbol{0}, \sigma_\omega^2 \boldsymbol{Q}^{-1}_\omega\right)
$$
...
<-- EW: as above, I changed epsilon to be subscripted with s throughout this section-->
IID spatiotemporal random fields (`spatiotemporal = 'iid'`) can be represented as

$$
\begin{aligned}
\mu_{\bm{s},t} &= f^{-1} \left( \ldots + \boldsymbol{\epsilon_{\s,t}} + \ldots \right),\\
\boldsymbol{\epsilon_{\s,t}} &\sim \operatorname{MVNormal} \left( \boldsymbol{0}, \boldsymbol{\Sigma}_{\epsilon} \right).
\end{aligned}
$$

where $\boldsymbol{\epsilon_{\s,t}}$ represent random field deviations at point $\s$ and time $t$.
...
$$
\boldsymbol{\epsilon_{\s,t}} \sim \mathrm{MVNormal}\left(\boldsymbol{0}, \sigma_\epsilon^2 \boldsymbol{Q}^{-1}_\epsilon\right)
$$
...
\boldsymbol{\delta}_{t>1} &= \rho \boldsymbol{\delta}_{t-1} + \sqrt{1 - \rho^2} \boldsymbol{\epsilon_{\s,t}},  \:
\boldsymbol{\epsilon_{\s,t}} \sim \operatorname{MVNormal} \left(\boldsymbol{0}, \boldsymbol{\Sigma}_{\epsilon} \right),
\end{aligned}
$$
...
\mu_{\s,t} &= f^{-1} \left( \ldots + \delta_{\s,t} + \ldots \right),\\
\boldsymbol{\delta}_{t=1} &\sim \operatorname{MVNormal} (\boldsymbol{0}, \boldsymbol{\Sigma}_{\epsilon}),\\
\boldsymbol{\delta}_{t>1} &= \boldsymbol{\delta}_{t-1} +  \boldsymbol{\epsilon_{\s,t-1}},  \:
\boldsymbol{\epsilon_{\s,t-1}} \sim \operatorname{MVNormal} \left(\boldsymbol{0}, \boldsymbol{\Sigma}_{\epsilon} \right),
\end{aligned}
$$
...
Currently, $\bm{X}_{3,t}$ is restricted to a single covariate (column vector), supplied by a one-sided formula to `spatial_varying`.
...
<-- EW: make alpha bold here, as above?-->
$$
\begin{aligned}
...
<-- EW: Put lme4 and glmmTMB in '' too?-->
...
$$
<-- EW: check this, I think the Gamma variance should be mu * phi2 -->
...
<-- EW: the lognormal definition above includes the bias correction phi2/2, but that's not in the standard implementations in R, etc. I think I used those R functions or referenced Wikipedia in writing the equations out. So we should pick one or the other. I think just changing the definition to not include the bias correction would keep it consistent with R, https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Lognormal.html-->
