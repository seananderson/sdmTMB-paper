...
* the Latin/Roman alphabet for data,

* bold symbols for vectors or matrices,
...
* $\mathrm{Var}[y]$ to define the expected variance of the variable $y$, and

* a $^*$ superscript to represent interpolated or projected values as opposed to values at knot locations (e.g., $\omega$ vs. $\omega^*$).
...
symbols <- tibble::tribble(
  ...
  "$f$", "Link function",
  "$f^{-1}$", "Inverse link function",
  "$\\boldsymbol{\\beta}$", "Parameter vector",
  "$\\boldsymbol{X}$", "A predictor model matrix",
  "$O_{\\s, t}$", "An offset variable at point $\\s$ and time $t$",
  
  "$\\omega_{\\s}$", "Spatial random field at point $\\s$ (knot)",
  "$\\omega_{\\s}^*$", "Spatial random field at point $\\s$ (interpolated)",
  "$\\zeta_{\\s, t}$", "Spatially varying coefficient random field at point $\\s$ and time $t$ (knot)",
  "$\\zeta_{\\s, t}^*$", "Spatially varying coefficient random field at point $\\s$ and time $t$ (interpolated)",
  "$\\epsilon_{\\s, t}$", "Spatiotemporal random field at point $\\s$ and time $t$ (knot)",
  "$\\epsilon_{\\s, t}^*$", "Spatiotemporal random field at point $\\s$ and time $t$ (interpolated)",
  "$\\delta_{\\s,t}$", "AR1 or random walk spatiotemporal deviations (knot)",
  "$\\alpha_g$", "IID random intercept deviation for group $g$",
  ...
  "$\\boldsymbol{Q}_\\omega$", "Spatial random field precision matrix",
  "$\\boldsymbol{Q}_\\zeta$", "Spatially varying coefficient random field precision matrix",
  "$\\boldsymbol{Q}_\\epsilon$", "Spatiotemporal random field precision matrix",
  
  "$\\sigma_\\alpha^2$", "IID random intercept variance",
  "$\\sigma_\\epsilon^2$", "Spatiotemporal random field marginal variance",
  "$\\sigma_\\omega^2$", "Spatial random field marginal variance",
  "$\\sigma_\\zeta^2$", "Spatially varying coefficient random field marginal variance",
  "$\\kappa_\\omega$", "Spatial decorrelation rate",
  "$\\kappa_\\epsilon$", "Spatiotemporal decorrelation rate",
  
  "$\\rho$", "Correlation between random fields in subsequent time steps",
  "$\\boldsymbol{A}$", "A sparse projection matrix to bilinearly interpolate between knot and data locations",
  "$\\boldsymbol{H}$", "A 2-parameter rotation matrix used to define anisotropy"
)
...
Similarly to the spatial random fields, these spatiotemporal random fields (and all versions described below) are parameterized internally with a sparse precision matrix ($\boldsymbol{Q}_\epsilon$):

$$
\boldsymbol{\epsilon}_t \sim \mathrm{MVNormal}\left(\boldsymbol{0}, \sigma_\epsilon^2 \boldsymbol{Q}^{-1}_\epsilon\right)
$$
...
with mean zero that varies by level $g$ and is constrained by
...
## [Lognormal](https://github.com/pbs-assess/sdmTMB/blob/18a39eabc111e2179fa589f942c8820d87ad10df/src/utils.h#L47-L54)
...
## [Negative Binomial 1 (NB1)](https://github.com/pbs-assess/sdmTMB/blob/18a39eabc111e2179fa589f942c8820d87ad10df/src/sdmTMB.cpp#L577-L582)
...
## [Student-t](https://github.com/pbs-assess/sdmTMB/blob/18a39eabc111e2179fa589f942c8820d87ad10df/src/utils.h#L37-L45)
...
The parameter $\nu$ is set to 1 to take advantage of the Stochastic Partial Differential Equation (SPDE) approximation to the GMRF to greatly increase computational efficiency [@lindgren2011].
...
SPDE models allow for barriers to be included via barrier meshes [@bakka2019]. 
...
The barrier mesh implementation requires the user to select a fraction value (`range_fraction` argument) that defines the fraction of the usual spatial range when crossing the barrier [@bakka2019]. 
...
Like AD Model Builder, TMB allows for parameters to be fit in phases and we include the `multiphase` argument in `sdmTMB::sdmTMBcontrol()` to allow this. 
For high-dimensional models (many of fixed and random effects), phased estimation may be faster and result in more stable convergence. 
