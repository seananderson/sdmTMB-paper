...
$$
\begin{aligned}
  \mu_{\s,t} &= f^{-1} \left( \ldots +  \bm{X}^{\mathrm{tvc}}_{\s,t} \gamma_{t} + \ldots \right),\\
...
where $\gamma_t$ is an example optional time-varying regression parameter and $\bm{X}^{\mathrm{tvc}}_{\s,t}$ is the corresponding predictor value.
...
Usually, $\bm{X}^{\mathrm{svc}}_{\s, t}$ would represent a predictor that is constant spatially for a given time $t$.
Currently, $\bm{X}^{\mathrm{svc}}_{t}$ is restricted to a single covariate (column vector), supplied by a one-sided formula to `spatial_varying`.
