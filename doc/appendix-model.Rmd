---
title: "Appendix 1: sdmTMB statistical model description"
output:
...
* $\mathbb{E}$ indicates expected value;
...
Next, we will split this model apart to describe the various parts in more detail using $\ldots$ to represent the other optional components.
...
This is the same approach as is implemented in the R packages gamm4 (REF) and brms (REF).
...
and $s95$ is a parameter controlling the point at which the function reaches 95%. 
...
This has been chosen to match the representation in VAST (REF). 
...
AR(1) spatiotemporal random fields (`spatiotemporal = 'ar1'`) add a parameter defining the correlation between random field deviations from one time slice to the next. 
They are defined as
...
The $\rho \boldsymbol{\delta}_{t-1} + \sqrt{1 - \rho^2}$ scales the spatiotemporal variance by the correlation such that it represents the steady-state marginal variance.
The correlation $\rho$ allows for mean-reverting spatiotemporal fields, and is constrained to be $-1 < \rho < 1$.
Internally, the parameter is estimated as `ar1_phi`, which is unconstrained. The parameter `ar1_phi` is transformed to $\rho$ with $\rho = 2 \left( \mathrm{logit}^{-1}(\texttt{ar1\_phi}) - 1 \right)$.
...
Usually, $\bm{X}_{3,t}$ would represent a predictor that is constant spatially for a given time $t$.
Currently, $\bm{X}_{3,t}$ is restricted to a single covariate, supplied by a one-sided formula to `spatial_varying`.
...
$$
where $\mu$ is the mean and $\phi$ is the standard deviation (following Stan convention of standard deviation not variance).
...
where $\mu$ and $\phi$ are the mean and standard deviation of the natural log of the random variable. 
If $\boldsymbol{x}$ is lognormally distributed, $\mathbb{E}[\log(\boldsymbol{x})] = \mu$ and $\mathrm{Var}[\log(\boldsymbol{x})] = \phi$.
This translates to $\mathbb{E}[\boldsymbol{x}] = \exp(\mu + \phi^2 / 2)$ and $\mathrm{Var}[\boldsymbol{x}] = \left(\exp(\phi^2) - 1 \right) \cdot \exp(2 \mu + \phi^2)$.

TODO: did this get reversed above? Isn't it set up so the mean(x) is mu?
...
The Tweedie distribution can be helpful for modelling data that positive and continuous but also contains zeros.

Internally, $p$ is transformed from $\mathrm{logit}^{-1} (\texttt{thetaf}) + 1$ to constrain it between 1 and 2 and \texttt{thetaf} is fit as an unconstrained variable.
...
Internally, the random fields are assumed to follow a Gaussian Markov random field. For example, the spatial random field case:

$$
\boldsymbol{\omega} \sim \mathrm{MVN}\left(\boldsymbol{0}, \sigma_\omega^2 \boldsymbol{Q}^{-1}\right)
$$
where $\boldsymbol{Q}$ is a sparse precision matrix.
  
## Projection '$\boldsymbol{A}$' matrix

The values of the spatial variables at the knots are multiplied by a projection matrix $\boldsymbol{A}$ that bilinearly interpolates the values at the locations of the observed or predicted data [@lindgren2015]:
...
Three non-zero elements on each row define the weight of the neighbouring 3 knot locations.
The same bilinear interpolation happens for any spatiotemporal random fields and $\boldsymbol{\epsilon}_t$.

## Anisoptropy

TMB allows for anisotropy, where spatial covariance may be asymmetric with respect to latitude and longitude ([full details](https://kaskr.github.io/adcomp/namespaceR__inla.html)). Anisotropy can be turned on and off with the boolean `anisotropy` argument to `sdmTMB()`. There are a number of ways to implement anisotropic covariance REF, and we adopt a 2-parameter rotation matrix $\textbf{H}$. The elements of $\textbf{H}$ are defined by the parameter vector $\textbf{x}$ so that $H_{1,1} = x_{1}$, $H_{1,2} = H_{2,1} = x_{2}$ and $H_{2,2} = (1 + x_{2}^2) / x_{1}$
...
In some cases the spatial domain of interest may be complex and bordered by land or water (e.g., coastlines, islands, lakes). 
SPDE models allow for barriers to be included via barrier meshes. 
...
Like AD Model Builder, TMB allows for parameters to be fit in phases and we include the `multiphase` argument in `sdmTMB::sdmTMBcontrol()` to allow this. For high dimensional models (lots of fixed and random effects), phased estimation may be faster and result in more stable convergence. In sdmTMB, phased estimation proceeds by first estimating all fixed effect parameters contributing to the likelihood (holding random effects constant at initial values). In the second phase, the random effect parameters (and their variances) are also estimated. Fixed effect parameters are also estimated in the second phase, and initialized at their estimates from the first phase.  
...
Much of the guidance around diagnostics and `glmmTMB` also applies to `sdmTMB`, e.g. [https://cran.r-project.org/web/packages/glmmTMB/vignettes/troubleshooting.html](https://cran.r-project.org/web/packages/glmmTMB/vignettes/troubleshooting.html). Warnings of non-positive-definite Hessian matrices (accompanied by parameters with NAs for standard errors) often mean models are improperly specified given the data used. Optimization with `stats::nlminb()` involves specifying the number of iterations and evaluations (`eval.max` and `iter.max`) and the tolerances (`abs.tol`, `rel.tol`, `x.tol`, `xf.tol`) -- more iterations and smaller tolerance thresholds ensure that the optimal solution is found, but more evaluations translates into longer computation time. Additionally, multiple function calls to `stats::nlminb()` may aid convergence; we include `nlmbinb_loops` as an argument to `sdmTMB()`, where each call is initialized at the best parameter vector from the previous iteration. Alternatively, `sdmTMB()` may be called repeatedly, using different random seeds.  
