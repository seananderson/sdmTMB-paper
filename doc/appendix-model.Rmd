...
In some cases the spatial domain of interest may be complex, and bordered by land or water (coastlines, islands, lakes, etc). SPDE models allow for barriers to be included, via barrier meshes. With  `sdmTMB()` models, the mesh construction occurs in two steps: (1) the user constructs a mesh with a call to `sdmTMB::make_mesh()`, and (2) passes the mesh to `sdmTMB::add_barrier_mesh()`. The barriers must be constructed as `sf` objects with polygons defining the barriers. See `?sdmTMB::add_barrier_mesh` for examples.
