...
For example, a mesh with a cutoff distance of 10 km can be made (Fig.\ \ref{fig:mesh}).

```{r mesh, echo = TRUE, cache=TRUE}
mesh <- make_mesh(pcod, xy_cols = c("X", "Y"), cutoff = 10)
```

```{r plot-mesh, cache=TRUE, fig.align='center', fig.asp=1, out.width="3in", echo=TRUE, fig.cap="This mesh uses a cutoff distance of 10 km.", fig.pos='h', fig.align='center'}
plot(mesh)
```
...
```{r sim-resids, out.width="4in", warning=FALSE, results='hide', cache=TRUE, echo=TRUE, fig.cap="Simulated residuals from the DHARMa package.", fig.pos='h', fig.align='center'}
set.seed(123)
simulate(fit, nsim = 500) %>%
  dharma_residuals(fit)
```

Or perhaps better, use MCMC residuals (Fig.\ \ref{fig:mcmc-resids}).
...
```{r mcmc-resids-vis, out.width="4in", fig.cap="MCMC residuals.", fig.pos='h', fig.align='center'}
qqnorm(r)
qqline(r)
```
...
```{r plot-all-effects, fig.cap="Model predictions based on all fixed effects and random effects.", fig.pos='h', fig.align='center'}
...
```{r plot-fix-defects, fig.cap="Predictions based on only the non-random field elements.", fig.pos='h', fig.align='center'}
...
```{r plot-spatial-effects, fig.cap="Spatial random effects only.", fig.pos='h', fig.align='center'}
...
```{r plot-spatiotemporal-effects, fig.cap="Spatiotemporal random effects only.", fig.pos='h', fig.align='center'}
...
```{r sims-cv-plot, cache=TRUE, fig.cap="Spatiotemporal coefficient of variation.", fig.pos='h', fig.align='center'}
...
```{r pcod-app-index, fig.asp=0.45, out.width="5in", cache=TRUE, fig.cap="Geostatistical population index.", fig.pos='h', fig.align='center'}
