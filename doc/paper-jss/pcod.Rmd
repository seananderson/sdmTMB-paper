...
We will fit a geostatistical spatiotemporal model with \pkg{sdmTMB} for the purposes of area-weighted biomass index standardization. We will use a data set built into the package: trawl survey data for Pacific Cod in Queen Charlotte Sound. The density units are kg/km^2^. Here, X and Y are UTM zone 9 coordinates in kilometers. If our spatial coordinates were not already in an equidistant projection, we would transform them [e.g., using `sf::st_transform()`, @pebesma2018] to ensure that distance remains constant throughout the study region or using the built-in `sdmTMB::add_utm_columns()` function.
...
These residuals can suggest model fit issues even with properly specified models due to the Laplace approximation [@thygesen2017a]. We could instead use MCMC residuals, which more accurate but slower. These are currently available in the \pkg{sdmTMBextra} package at <https://github.com/pbs-assess/sdmTMBextra> and are demonstrated in the [residuals vignette](https://pbs-assess.github.io/sdmTMB/articles/).
