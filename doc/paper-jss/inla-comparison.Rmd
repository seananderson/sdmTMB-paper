...
```{r setup-inla-comparison, include = FALSE, cache=FALSE}
...
```{r coef-plot, fig.asp=0.4, out.width="85%", echo=FALSE}
...
From \pkg{INLA}, we can extract the marginal posterior densities using `inla.smarginal()` and name them to match the variable names from the \pkg{sdmTMB} `gather_sims()` function.
...
We can predict on a grid (`qcs_grid`) provided with the `pcod` data in the \pkg{sdmTMB} package to map occurrence probabilities in each year in space.
...
We can get similar predictions with \pkg{inlabru} using only a few additional steps: we must recreate `i_year`, create a `SpatialPixelsDataFrame` class data frame, and specify in `formula` to include both the random field (`f`), the factors (`fac`), and the appropriate inverse-link transformation:
