...
title:
  formatted: "\\pkg{sdmTMB}: An \\proglang{R} Package for Fast, Flexible, and Accessible Generalized Linear Mixed Effects Models with Spatial and Spatiotemporal Random Fields"
...
\begin{landscape}
\begin{figure}[p]
{\centering \includegraphics[width=9in]{../../figs/diagram}}
\caption[]{
...
And we can run some basic checks on our model with the `sanity()` function:
...
\begin{figure}
{\centering \includegraphics[width=5.2in]{../../figs/pcod-fig}}
...
fit_spatiotemporal <- sdmTMB(
  ...
  anisotropy = TRUE,
  silent = FALSE # print fitting progress
)
```
...
We can inspect the anisotropy with `plot_anisotropy()`, which plots an ellipse showing the spatial range with the spatial coordinates centered on zero.
...
\begin{figure}
{\centering \includegraphics[width=3.5in]{../../figs/timing-spatial-m1-april11}}
