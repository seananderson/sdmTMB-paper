...
See the model description vignette for a full description of the statistical model (`browseVignettes("sdmTMB")` or see the \href{https://pbs-assess.github.io/sdmTMB/articles/}{rendered version}).
...
The \pkg{sdmTMB} package is constructed from S3 classes and designed to be both modular and familiar to users of widely used \proglang{R} packages (e.g. \pkg{glmmTMB}, \pkg{lme4}, \pkg{mgcv}). Data may be simulated with or without a fitted model (`simulate.sdmTMB()`, `sdmTMB_simulate()` respectively). Estimation is done via maximum marginal likelihood with the `sdmTMB()` function (or `sdmTMB_cv()` for cross-validation), and the resulting fitted model can be used to make predictions (`predict.sdmTMB()`) or calculate residuals (`residuals.sdmTMB()`), derive additional quantities of interest, and simulate Bayesian posterior distributions. Common quantities of interest may include summarizing shifts in the center of gravity (`get_cog()`) [e.g., @thorson2016cog] or in the field of ecology, tracking total population densities through time (`get_index()`).
...
## Illustrating the need for spatial random fields

We begin with a simple species distribution model of encounter probability of
Pacific Cod (TODO spp) from a trawl survey conducted off the west
coast of Vancouver Island, British Columbia, Canada. This survey consists of spatially random sampling in each year, resulting in a georeferenced dataset. For compatibility with other packages, spatial random fields in \pkg{sdmTMB} can be toggled on and off. We will fit a logistic
regression of encounter probability with and without spatial random fields to
illustrate the importance of accounting for spatial correlation. In addition to the spatial random field, we include an intercept and a quadratic effect of depth on the probability of occurrence. 
...
We can create a 'mesh' for the SPDE calculations with `make_mesh()`.

```{r dog-binomial, results='hide', message=FALSE, warning=FALSE, echo=TRUE}
mesh_pcod <- make_mesh(pcod, xy_cols = c("X", "Y"), cutoff = 8)
plot(mesh_pcod)
```

We will fit a model with and without spatial random fields by toggling the `spatial` argument from `"on"` to `"off`. The `update()` S3 method refits the same model updating any specified arguments.
...
We can get a summary of our model fit by printing it.

```{r dog-bin-summary, echo=TRUE}
fit_bin_rf
```

Describe what we see here

We can test for spatial autocorrelation with a Moran's I test or with a visual inspection of the residuals.
...
test_autocor(fit_bin)
test_autocor(fit_bin_rf)
```

We can also see that the AIC of the model with spatial random fields is considerably lower:

```{r}
AIC(fit_bin_rf, fit_bin)
```

We could do additional testing with cross validation via `sdmTMB_cv()`. Here we will do 10-fold cross validation with the folds constructed randomly. Using the `fold_ids` argument, we could supply our own folds and conduct spatially blocked cross validation, for example.

```{r}
cv_bin_rf <- sdmTMB_cv(present ~ poly(log(depth), 2),
  data = pcod, mesh = mesh_pcod, spatial = "on",
  family = binomial(link = "logit"), k_folds = 10
)
cv_bin_rf <- sdmTMB_cv(present ~ poly(log(depth), 2),
...
The standard errors on our fixed effects have increased:

```{r}
tidy(fit_bin_rf, conf.int = T)
tidy(fit_bin, conf.int = T)
```

We now have an effect of depth and a spatial random field, which combined in link space result in our overall prediction. Note the difference between only the fixed effects (left) and including the spatial random field (right).
...
g1 <- plot_spatial_map(p, plogis(est_non_rf), "Fixed effects")
g2 <- plot_spatial_map(p, omega_s, "Spatial random field") +  scale_fill_gradient2()
g3 <- plot_spatial_map(p, plogis(est), "Combined prediction")
...
As a second example, we will construct a model of catch rates of North Pacific spiny dogfish from the same survey. This example extends the spatial model described previously by also including (1) spatiotemporal fields allowing the distribution in each year to be unique (modelled as an autoregressive process, allowing hotspots to persist through time), (2) a time varying intercept to allow each year to have a different mean catch rate, and (3) a smooth effect of ln(depth), allowing catch rates to vary non-linearly with depth. As catch rates are both skewed and zero inflated, we specify the response family as a Tweedie distribution [@tweedie1984].

```{r dog-head-dat, echo=TRUE}
dat <- select(dogfish, longitude, latitude, year, catch_weight, area_swept, depth)
head(dat)
```

```{r dog-utms, echo=TRUE}
dat <- add_utm_columns(dat, c("longitude", "latitude"), units = "km", utm_crs = 32609)
dat$log_depth <- log(dat$depth)
```

```{r dog-mesh, echo=TRUE}
mesh <- make_mesh(dat, xy_cols = c("X", "Y"), cutoff = 8)
```

```{r dog-mesh2, eval=FALSE}
plot(mesh)
```

```{r dog-tw, results='hide', message=FALSE, warning=FALSE, echo=TRUE}
fit_tw <- sdmTMB(
...
We can use `update()` to refit our model quickly without specifying all of the arguments. This is highlighted by exploring models with four additional families, each using a delta- or hurdle distribution:
delta-gamma, delta-lognormal, delta-Poisson-link-gamma, and
delta-Poisson-link-lognormal.

```{r dog-update, results='hide', message=FALSE, warning=FALSE, echo=TRUE}
fit_dg <- update(fit_tw, family = delta_gamma(), spatiotemporal = list("off", "iid"))
...
We can then compare the fits via AIC:
...
We find that the Poisson-link-lognormal model is favoured with AIC. We next
test two additional model formulations: making the spatial correlation
isotropic (the default) instead of anisotropic and structuring the positive model component
spatiotemporal random fields as AR(1).

```{r dog-ar1, results='hide', echo=TRUE}
fit_dpl_iso <- update(fit_dpl, anisotropy = FALSE)
fit_dpl_ar1 <- update(fit_dpl, spatiotemporal = list("off", "ar1"))
```

```{r dog-aci2, echo=TRUE}
AIC(fit_dpl_ar1, fit_dpl, fit_dpl_iso)
```

<!-- Need description of anisotropy and why it's important here or somewhere else -->
We find that the anisotropic AR(1) is favoured. On inspecting the model output
with `summary()`, we notice that the spatial random field SD in the positive model component has become very
small once AR(1) spatiotemporal fields were added. We therefore consider
a model where these are turned off in the second component of the delta model:

```{r dog-ar1-2, echo=TRUE, results="hide"}
fit_dpl_ar1_only <- update(fit_dpl_ar1, spatial = list("on", "off"))
```

```{r dog-aic3, echo=TRUE}
AIC(fit_dpl_ar1_only, fit_dpl_ar1, fit_dpl)
```

```{r dog-print, echo=TRUE}
fit <- fit_dpl_ar1_only
sanity(fit)
summary(fit)
```

```{r dog-aniso, echo=TRUE}
plot_anisotropy(fit_dpl)
```

<!-- Propose moving this to a short paragraph `Prediction` to highlight modularity -->
```{r dog-grid, echo=TRUE}
head(wcvi_grid, n = 3)
...
```{r dog-pred1, echo=TRUE}
pred <- predict(fit, newdata = grid, type = "response")
```

Our prediction data frame has..., which we can easily plot with ggplot2 or similar. We suppress that code in this paper for brevity.

```{r dog-pred2, echo=TRUE}
names(pred)
```
...
```{r dog-depth, echo=TRUE}
nd <- data.frame(
  log_depth = seq(min(dat$log_depth), max(dat$log_depth), length.out = 100), 
  year = max(dat$year)
)
pred_depth <- predict(fit, newdata = nd, se_fit = TRUE, re_form = NA, model = NA)
```

```{r dog-depth-plot, echo=FALSE}
...
We can write the model as TODO

```{r owl-data}
snow <- readRDS(here::here("data/SNOW_data.rds"))
```

```{r owl-fit, echo=TRUE, results = "hide"}
...
```{r owl-sanity, eval=FALSE}
sanity(fit_owl)
```

```{r owl-print, echo=TRUE}
summary(fit_owl)
```

The SVC effect at any point in space is the combination of the fixed effect and spatial random effect for `nao`. Here we illustrate

We can extract the SVC effect with TODO

```{r p-mean, message=FALSE, echo=TRUE, eval=TRUE, cache=TRUE}
nsim <- 200
zeta_s <- predict(fit_owl, newdata = snow, nsim = nsim, sims_var = "zeta_s")
...
```{r owl-plot-basic, echo=TRUE, eval=FALSE}
ggplot(snow, aes(X, Y)) +
  geom_point(aes(colour = nao_effect)) +
  scale_colour_viridis_c()
```

Overall, we found a weak average positive effect of annual mean NAO on overall counts, but a southeast to northwest gradient in the intensity of the effect (Figure \ref{fig:owl-nao}).
...
```{r owl-plot-fancy, echo=FALSE, eval=TRUE, fig.cap="Map of spatially varying coefficient effects of NAO on Snowy Owl count. In the west, the lower values are near 1 implying no effect, whereas in the southeast the effect becomes positive. Points represent all count locations."}
...
Importantly, `dependencies = TRUE` includes the \proglang{R}-INLA package  [@rue2009; @lindgren2011; @lindgren2015], which is not on CRAN but is needed for the `make_mesh()` function below.
...
```{r sdmTMB-lib0, eval=FALSE, echo=FALSE}
# library("sdmTMB")
# # simplify for paper:
...
```{r sdmTMB-lib, eval=FALSE, echo=TRUE}
library("sdmTMB")
dplyr::select(pcod, year, density, depth, lon, lat) |>
  head(n = 3)
```   
...
```{r pcod-utms-eval, echo=TRUE, eval=FALSE}
...
```{r pcod-head2, echo=TRUE, eval=FALSE}
head(pcod, n = 3)
```
...
We can inspect our mesh object with the associated plotting method (`plot(mesh)`; see Figure \ref{fig:pcod-fig}A).
...
```{r pcod-eg1-fit, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE, cache=TRUE}
fit <- sdmTMB(
  density ~ s(depth),
  data = pcod,
...
```{r pcod-eg1-summary,eval=FALSE, echo=TRUE}
fit
```
...
```{r pcod-eg1-tidy-fe, eval=FALSE, echo=TRUE}
tidy(fit, conf.int = TRUE)
```

```{r pcod-eg1-tidy-re, eval=FALSE, echo=TRUE}
...
```{r pcod-eg1-sanity, eval=FALSE, echo=TRUE}
sanity(fit)
```
...
```{r pcod-eg1-suppress-depth2, eval=FALSE, echo=FALSE}
...
```{r qcs-head, echo=FALSE}
head(qcs_grid, n = 3)
```

```{r pcod-eg1-predict, echo=FALSE, eval=TRUE, cache=TRUE}
...
```{r pcod-p-tibble, eval=FALSE}
p <- tibble::as_tibble(p)
```

```{r pcod-head-predict, echo=TRUE, eval=FALSE}
head(p, n = 3)
```

We show a basic plot of the estimated spatial random field, predictions across a depth gradient, and predictions in space in Figure \ref{fig:pcod-fig}B--D.
...
```{r time1, cache=TRUE, eval=FALSE}
t1 <- Sys.time()
```

```{r fit-tv, eval=FALSE, echo=TRUE, cache=TRUE, results='hide'}
...
```{r time2, cache=TRUE, eval=FALSE}
t2 <- Sys.time()
```

```{r fit-tv-print, eval=FALSE, echo=TRUE}
fit_spatiotemporal
```
...
```{r owl-fit-old, warning=FALSE, message=FALSE, cache=TRUE, echo=TRUE, eval=FALSE}
...
We found a weak average positive effect of annual mean NAO on overall counts, but a southeast to northwest gradient in the intensity of the effect (Figure \ref{fig:owl-nao}).
This result is consistent with owls closest to the Atlantic coast and those migrating the furthest south being the most affected by NAO.
We provide an extended version of this example in Appendix \ref{app:owls}.
...
```{r child="pcod.Rmd", eval=FALSE, cache=TRUE}
```

```{r child="owls.Rmd", eval=FALSE, cache=TRUE}
```

```{r child="inla-comparison.Rmd", eval=FALSE, cache=TRUE}
```

```{r child="timing.Rmd", eval=FALSE, cache=TRUE}
```
