...
abstract: >
  Geostatistical data are common across disciplines and available in increasingly large volumes. However, appropriate models to analyse these data, such as generalised linear mixed effects models (GLMMs) with Gaussian random fields, are computationally intensive and challenging to implement such that spatial autocorrelation is frequently ignored, or addressed in less interpretable ways. Here, we introduce the \proglang{R} package \pkg{sdmTMB}, which extends the flexible interface familiar to users of `stats::glm()`, \pkg{lme4}, \pkg{glmmTMB}, or \pkg{mgcv} to include spatial and spatiotemporal latent fields using a predictive-process SPDE-(stochastic partial differential equation) based approach. SPDE matrices are constructed with \pkg{INLA} and estimation is conducted via maximum marginal likelihood with Template Model Builder (\pkg{TMB}), although penalised likelihood and Bayesian inference (via \pkg{tmbstan} and \pkg{rstan}) are also possible. We describe the statistical model, illustrate the package through two case studies, and compare the functionality, speed, and interface to related software. The case studies illustrate \pkg{sdmTMB}'s flexibility and useability by implementing penalised smoothers, non-stationary processes (time-varying and spatially varying coefficients), and anisotropy (asymmetry in spatial correlations). Modelling of these common but computationally complex relationships is further aided by built-in handling of delta (hurdle) models, break-point effects, and diagnostic tools including out-of-sample cross validation. Finally, we demonstrate that \pkg{sdmTMB} is 4--16 times faster than \pkg{INLA} or \pkg{inlabru}. \pkg{sdmTMB}'s accessible interface has already opened this useful class of models to a wider audience within ecological species distribution modelling. We anticipate its application to the full spectrum geostatistical data will prove equally fruitful.
keywords:
...
One common application in the field of ecology is species distribution models (SDMs), hence the package name.
