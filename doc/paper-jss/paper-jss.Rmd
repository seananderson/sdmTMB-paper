...
Further testing could be done with cross validation via `sdmTMB_cv()`.
...
cv_bin_rf <- sdmTMB_cv(present ~ poly(log(depth), 2),
  data = dat, mesh = mesh, spatial = "off",
  family = binomial(link = "logit"), k_folds = 10
)
```
...
isotropic (the default) instead of anisotropic and structuring the
...
with `summary()`, we notice that the spatial random field SD has become very
...
```{r dog-plot1, echo=TRUE}
pred |> filter(year %in% c(2004, 2014, 2022)) |> 
  plot_map(est) +
  labs(fill = "Density") +
  scale_fill_viridis_c(trans = "log10")
```

```{r dog-plot2, echo=TRUE}
pred |> filter(year %in% 2004) |> 
  plot_map(omega_s2) +
  scale_fill_gradient2()
```

```{r dog-plot3, echo=TRUE}
pred |> filter(year %in% c(2004, 2022)) |> 
  plot_map(epsilon_st2) +
  scale_fill_gradient2()
```

```{r dog-plot4, echo=TRUE}
pred |> filter(year %in% 2004) |> 
...
Overall, we found a weak average positive effect of annual mean NAO on overall counts, but a southeast to northwest gradient in the intensity of the effect (Fig.\ \ref{fig:owl-nao}).
