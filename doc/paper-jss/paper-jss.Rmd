...
```{r remotes, eval=FALSE, echo=TRUE}
...
illustrate the need for spatial random fields. This survey conducts
depth-stratified random sampling biennially, resulting in a georeferenced
dataset. The dataset includes columns for year, latitude, longitude, Universal
Transverse Mercator (UTM) zone 9 coordinates, bottom depth, and encounter vs.
non-encounter.
...
```{r pcod-head, echo=TRUE}
select(pcod, year, lat, lon, X, Y, depth, present) |> 
  head(n = 3)
```

An \pkg{sdmTMB} model requires a data frame that contains a response column,
columns for any predictors, and columns for spatial coordinates.
Usually it makes sense to convert the spatial coordinates to an equidistant
projection such as the Universal Transverse Mercator (UTM) to ensure that
distance remains constant throughout the study region [e.g., using
`sf::st_transform()`, @pebesma2018]. Here we use the helper function
`add_utm_columns()` to add UTM coordinates with km units (so our estimated
spatial range parameter is not too big or small). By default, the function will
guess the UTM zone and create new columns `X` and `Y`. Since our example
data set already has these UTM columns, we can skip running this code.
...
`make_mesh()` is a wrapper function for `INLA::inla.mesh.create()` and `cutoff`
is passed through to it. Soon, \pkg{sdmTMB} will switch to using the
\pkg{fmesher} package for this purpose, which is hosted on CRAN. Alternatively,
we could have created any mesh via \proglang{R}-\pkg{INLA} or \pkg{fmesher} and
supplied it to the `mesh` argument in `make_mesh()`. We can inspect our mesh
object with the associated plotting method.
...
Our random field model can be written as TODO.
...
```{r pcod-eg1-sanity, eval=TRUE, echo=TRUE, message=FALSE}
sanity(fit)
```
...
definite, that no extreme (XX) or small (XX) eigenvalues are detected, that no
absolute log likelihood gradients with respect two fixed defects are >= 0.001,
that all fixed effects have reported standard errors and that they do not look
unreasonably large (XX by default), that random field marginal standard
deviations are not unexpectedly small or large, and that the random field
Matérn range parameter does not look unreasonably large.

We can get a summary of our model fit:

```{r pcod-bin-summary, echo=TRUE}
summary(fit_bin_rf)
```
...
omit the Moran's I code for brevity but include it in the code supplement. The
p-value for spatial autocorrelation is low for the model without a random field
(p < 0.01) but large for the model with a random field (p = `r p_rf`)
suggesting significant residual correlation in the simpler model that is
alleviated by including the random field. 

We can also see that the AIC of the model with spatial random fields is
considerably lower:

```{r pcod-aic, eval=TRUE, echo=TRUE, results='markup'}
...
```{r pcod-cv-out, eval=TRUE, echo=TRUE, results='markup'}
...
```{r pcod-tidy, eval=TRUE, echo=TRUE, results='markup'}
...
```{r pcod-predict, echo=TRUE, results='markup'}
p <- predict(fit_bin_rf, newdata = qcs_grid)
select(p, X, Y, depth, est, est_non_rf, omega_s) |>
  as_tibble() |> head(n = 2)
```

TODO MOVE TO CAPTION? We have an effect of depth and a spatial random field,
which combined in link space result in our overall prediction. Note the
difference between only the fixed effects (left) and including the spatial
random field (right). TODO DESCRIBE PLOTTING AND/OR INCLUDE

```{r pcod-predict-maps, fig.width=11, fig.asp=0.3, out.width="6in", fig.cap="TODO"}
...
AR(1) process to allow each year to have a different mean catch rate, (3)
...
structures, and the inclusion of anisotropy to illustrate the flexibility of
\pkg{sdmTMB}.
...
dat <- select(dogfish, longitude, latitude, year, 
...
delta-Poisson-link-lognormal (REF). Note that we choose to omit the
spatiotemporal random fields from the first model component 
...
```{r dog-aniso, echo=TRUE, fig.cap="TODO", out.width="4in"}
...
Our prediction data frame is similar to the binomial spatial model, but
includes columns for the two delta model components (labelled with suffixes `1`
and `2`) and adds an `epsilon_st` column for spatiotemporal random effects. We can
easily generate plots from this data frame with ggplot2 or similar. We suppress
that code in this paper for brevity.
...
```{r dog-wvci-pred, fig.asp = 0.5, fig.cap="Example prediction components... TODO", fig.width=9, out.width="6in"}
cowplot::plot_grid(
  g_nonrf,
  g_omega,
  g_eps,
...
```{r dog-depth, echo=TRUE, out.width="5in"}
nd <- data.frame(
...
```{r dog-depth-plot, echo=FALSE, fig.cap="TODO", out.width="4in"}
...
```{r dog-index-plot, echo=FALSE, fig.cap="TODO", out.width="4in"}
...
combination of the main effect and spatial random effect for `nao`. Here we
...
a matrix of draws from the overall prediction. Here, we we instead specify that
we want to return draws from the `zeta_s` random field, which is the spatially
...
```{r owl-plot-fancy, echo=FALSE, eval=TRUE, fig.cap="Spatially varying coefficient for effect of mean annual NAO (North Atlantic Oscillation) on counts of Snowy Owls observed on annual Christmas Bird Counts 1979--2020 in Canada and the US. Points represent all count locations and circle area is scaled to the mean number of owls observed per year (range: 0 to 8). The effect is multiplicative on owl count per NAO unit. In the west, the lower values are near 1 implying no effect, whereas in the southeast the effect becomes positive."}
