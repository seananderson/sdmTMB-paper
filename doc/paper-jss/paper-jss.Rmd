...
The SPDE approach is a solution popularized via the \proglang{R}-\pkg{INLA} \proglang{R} package [@rue2009; @lindgren2011; @lindgren2015] and an implementation in \pkg{TMB} [Template Model Builder, @kristensen2016] that partially relies on \proglang{R}\-pkg{INLA} to create input matrices [e.g., @thorson2019; @osgood-zimmerman2021].
...
The Matérn can accommodate a range of shapes and can be both isotropic (covariance decays the same in all directions) or anisotropic (covariance is directionally dependent) [@haskard2007].
...
Here, we will predict on a 2 $\times$ 2 km grid (`qcs_grid`) that covers the entire region of interest so we can visualize the predictions spatially (or calculate a standardized population index with a spatiotemporal model).
...
We provide comparisons of the syntax and the reproducibility of results from models fit using \proglang{R}-\pkg{INLA} and \pkg{inlabru} in Appendix \ref{app:inla}.
...
Speed-wise, \pkg{sdmTMB} (and due to the same \pkg{TMB} backend, \pkg{VAST}) were fastest up to at least 1000 mesh nodes at approximately 4--16 times faster than \proglang{R}-\pkg{INLA}/\pkg{inlabru} across the range of mesh complexities considered here.
...
<!-- Not sure what kind of refs you want here?  \proglang{R}-\pkg{INLA}'s non-gaussian random fields are mentioned here @bakka2018review-->
