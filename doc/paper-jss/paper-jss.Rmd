...
author:
  - name: Sean C. Anderson
    ...
    affiliation: 'Pacific Biological Station, Fisheries and Oceans Canada \AND'
    address: |
      | Pacific Biological Station, Nanaimo, BC, Canada
    email: \email{sean.anderson@dfo-mpo.gc.ca}
  - name: Eric J. Ward
    orcid: 0000-0002-4359-0296
    affiliation: 'Northwest Fisheries Science Center, National Marine Fisheries Service \AND'
...
abstract: >
  Geostatistical data are pervasive and available in increasingly large volumes. However, appropriate models to analyse these data, such as generalised linear mixed effects models (GLMMs) with Gaussian random fields, are computationally intensive and challenging to implement. Here, we introduce the \proglang{R} package \pkg{sdmTMB}, which extends the flexible interface familiar to users of \pkg{lme4}, \pkg{glmmTMB}, or \pkg{mgcv} to include spatial and spatiotemporal latent fields using a predictive-process SPDE-(stochastic partial differential equation) based approach. SPDE matrices are constructed with \pkg{INLA} and estimation is conducted via maximum marginal likelihood with Template Model Builder (\pkg{TMB}), although penalised likelihood and Bayesian inference (via \pkg{tmbstan} and \pkg{rstan}) are possible. We describe the statistical model, illustrate the package through two case studies, and compare the functionality, speed, and interface to related software. The case studies illustrate \pkg{sdmTMB}'s flexibility and useability by implementing penalised smoothers, non-stationary processes (time-varying and spatially varying coefficients), and anisotropy (asymmetry in spatial correlations). Additional functionality includes delta (hurdle) models, break-point effects, and out-of-sample cross validation. Finally, we demonstrate that \pkg{sdmTMB} is 4--16 times faster than \pkg{INLA} or \pkg{inlabru}. \pkg{sdmTMB}'s accessible interface has already opened this useful class of models to a wider audience within ecological species distribution modelling (SDMs). We anticipate its application to the full spectrum of geostatistical data will prove equally fruitful.
keywords:
...
While such data are a rich source of information about mechanistic processes [@legendre1989a; @rossi1992; @tilman1997spatial], they are challenging to properly model---data closer in space and time are usually more similar to each other than data farther apart due to measured and unmeasured variables [@cressie1993; @diggle2007; @cressie2011].
...
Accounting for this residual correlation is important because doing so allows for valid statistical inference [@legendre1989a; @dormann2007], can improve predictions  [e.g., @shelton2014], and can be of interest itself by, for example, identifying locations with shared responses [e.g., @thorson2019d; @barnett2021].
...
The SPDE approach is a solution popularized via the \pkg{INLA} \proglang{R} package [@rue2009; @lindgren2011; @lindgren2015] and an implementation in \pkg{TMB} [Template Model Builder, @kristensen2016] that partially relies on \pkg{INLA} to create input matrices [e.g., @osgood-zimmerman2021].
...
Systems for specifying statistical models that can include the SPDE, such as \pkg{INLA} and \pkg{TMB}, are flexible and powerful but are challenging to use for many applied researchers.
...
Here, we introduce the \proglang{R} package \pkg{sdmTMB}, which implements geostatistical spatial and spatiotemporal GLMMs using \pkg{TMB} for model fitting and \pkg{INLA} to set up SPDE matrices.
...
One common application in the field of ecology is species distribution models (SDMs), hence the package name (i.e., SDMs with \pkg{TMB}: sdmTMB).
...
In contrast, semi- and non-parametric approaches do not estimate spatial covariance functions (e.g., randomForest [@liaw2002], MaxEnt [@phillips2006], and most smooths in \pkg{mgcv} [@wood2017a]).
...
The smoothers in \pkg{sdmTMB} can mimic most `mgcv::s()` smoothers including bivarate smoothers (`s(x, y)`), smoothers varying by continuous or categorical variables (`s(x1, by = x2)`), cyclical smoothers `s(x, bs = "cc")`, and smoothers with specified basis dimensions `s(x, k = 4)` [@wood2017a].
...
After rapid model exploration with maximum likelihood, one can optionally pass an \pkg{sdmTMB} model to the \proglang{R} package \pkg{tmbstan} [@monnahan2018] to estimate the joint posterior distribution for Bayesian inference.

\pkg{sdmTMB} models can include penalized likelihoods by assigning priors to model parameters (`?\pkg{sdmTMB}priors`).
...
For example, \pkg{sdmTMB} provides functionality for generating population-level summaries for each time slice including the center of gravity  (`get_cog()`) [e.g., @thorson2016cog] and total population index (`get_index()`) given a user-supplied grid.
...
Note that \texttt{s()} denotes a smoother as in \pkg{mgcv} [@wood2017a], \texttt{breakpt()} denotes a breakpoint 'hockey-stick' shape \citep[e.g.][]{barrowman2000}, \texttt{(1|g)} denotes a random intercept by group \texttt{g}, and \texttt{~ 0} is used in an \proglang{R} formula to omit an intercept.}\label{fig:diagram}
\end{figure}
\end{landscape}
...
It usually makes sense to convert the spatial coordinates to an equidistant projection such as the Universal Transverse Mercator (UTM) to ensure that distance remains constant throughout the study region [e.g., using `sf::st_transform()`, @pebesma2018, or `sdmTMB::add_utm_columns()`].
...
Alternatively, we could have created any mesh via the \pkg{INLA} package and supplied it to `make_mesh()`.
...
As noted in the summary method, we can extract a tidy data frame similar to using the \pkg{broom} (REF) or \pkg{broom.mixed} (REF) packages:
...
We could extend this spatial model to be a spatiotemporal one simply by supplying the year column name to the `time` argument (`time = "year"`) and specifying how we want the spatiotemporal random fields to be structured.
Here we will keep the spatial field (`spatial = "on"`), structure the spatiotemporal random fields as independent (`spatiotemporal = "iid"`), include a time-varying intercept (`~ 1`) fit as an AR(1) process (`time_varying_type = "ar1"`), include anisotropy to allow spatial correlation to decay at different rates up the coast vs.\ away from the coast (`anisotropy = TRUE`), and use `silent = FALSE` to see model fitting progress.
...
This model took approximately `r round(as.numeric(t2 - t1), 0)` seconds to fit on our laptop. 
...
A version that uses Markov chain Monte Carlo (MCMC) to avoid this issue is recommended but slower (`?residuals.sdmTMB`).
Simulation-based residuals [@dharma] (`dharma_residuals()`) are also possible.
...
\pkg{sdmTMB}, \pkg{VAST}, \pkg{INLA}/\pkg{inlabru}, and \pkg{spaMM} [@rousset2014] are the most closely related, as they all provide a user interface to SPDE-based GRF models; we also include \pkg{spBayes} in our software comparison as it is a prominent package that can fit related predictive-process models without the SPDE (Table 1).
\pkg{sdmTMB}, \pkg{VAST}, and \pkg{mgcv} can estimate anisotropic covariance whereas \pkg{INLA}/\pkg{inlabru} and \pkg{spBayes} are currently limited to isotropic covariance.
\pkg{sdmTMB} and \pkg{mgcv} focus on univariate response data, whereas \pkg{VAST}, \pkg{INLA}/\pkg{inlabru}, \pkg{spaMM}, and \pkg{spBayes} extend to multivariate responses with various limitations.
...
<!-- However, \pkg{VAST} is typically less effective at replicating nonlinear preferences than GAMs, which are supported by several of these packages [@brodie2020]. -->
Of these packages, only \pkg{sdmTMB} and \pkg{inlabru} can currently fit threshold (e.g., hockey-stick) covariate relationships.
\pkg{spaMM} is limited to a spatial random field and \pkg{spBayes} implements spatiotemporal fields, but only as a random walk.
There is considerable variability in the available observation likelihoods across packages (Table 1).
We provide comparisons of the syntax and the reproducibility of results from models fit using \pkg{INLA} or \pkg{inlabru} (Appendix 4) and \pkg{VAST} (Appendix 5).
...
\toprule
 & \pkg{sdmTMB} & \pkg{VAST} & \pkg{INLA}/\pkg{inlabru} & \pkg{mgcv} & \pkg{spBayes} & \pkg{spaMM}\\
\midrule
...
\begin{tablenotes}
\item \textit{Note: } 
\item $^1$Technically possible but non-trivial. $^2$Penalized smoother GAMs that determine `wiggliness'. $^3$\pkg{inlabru} but not \pkg{INLA}. $^4$Spatiotemporal fields as random walk only. $^5$SPDE approach as in Miller et al. (2019). $^6$Does have predictive process knots. $^7$Zero-inflated NB2 only. $^8$Tweedie power parameter fixed for \texttt{mgcv::gamm()}. $^9$Possible as log-linked Poisson GLMM with aggregated data. $^{10}$Hurdle models possible by fitting components separately.
\end{tablenotes}
\end{threeparttable}
\end{table}


<!-- TODO: Line 242. Can \pkg{sdmTMB} be run in parallel? If so, does the parallelism share memory across the cores or does the memory grow with the number of cores? This is a HUGE feature of PARDISO within \pkg{INLA}/\pkg{inlabru} allowing for very large models to be fit on HPCs. It might be worth mentioning and adding to Table 1. Also Line 242: do you expect the same relative performance between the software packages as more cores are used? -->

We ran a simple speed comparison between \pkg{sdmTMB}, \pkg{INLA}/\pkg{inlabru}, and \pkg{mgcv} for fitting an SPDE spatial random field model to 1000 data points with Gaussian error across a range of mesh resolutions (Fig. \@ref(fig:timing), Appendix 6).
In this test, \pkg{sdmTMB} was the fastest across all mesh resolutions; however, \pkg{INLA}/\pkg{inlabru} and \pkg{spaMM} were less affected by mesh resolution than \pkg{sdmTMB}.
...
\begin{figure}
\centering
\includegraphics[width=3.5in]{../../figs/timing-spatial-m1-april11}
\caption[]{Comparison of time to fit an SPDE spatial random field model with 1000 observations, an intercept and one predictor, Gaussian error, and a sequence of SPDE resolutions. Lines represent means and ribbons 95% quantiles across 50 random iterations. Note the log x and y axes. \pkg{VAST} is similar to \pkg{sdmTMB} and so is not shown. \pkg{inlabru} used the empirical Bayes integration strategy and Gaussian approximation with `bru_max_iter = 1`, and the `like()` formulation. \pkg{mgcv} used \texttt{bam()}, \texttt{method = 'fREML'}, and discretized covariates [@miller2019]. Note that spaMM only fits spatial, not spatiotemporal, models.
All platforms were restricted to one core and could be faster with parallel computation or optimized algebra libraries.}\label{fig:timing}
\end{figure}
...
We think users familiar with `stats::glm()`, \pkg{lme4}, or \pkg{glmmTMB} will find \pkg{sdmTMB} most approachable.
Users familiar with \pkg{INLA} will find \pkg{inlabru} approachable.
Users familiar with \pkg{mgcv} can adapt \pkg{mgcv} to fit similar models with custom code [@miller2019] and \pkg{INLA}/\pkg{inlabru} and \pkg{mgcv} are also general purpose modelling packages.
\pkg{VAST} is the sole option for fitting some multivariate models;
alternatively, because \pkg{VAST} focuses on multivariate delta models and fisheries applications (Appendix 5), users fitting "simple" univariate spatial/spatiotemporal GLMMs in non-fisheries contexts may find \pkg{sdmTMB} more straightforward to use.
...
Speed-wise, \pkg{sdmTMB} (and by association \pkg{VAST}) were fastest up to at least 1000 mesh nodes at approximately 4--16 times faster than \pkg{INLA}/\pkg{inlabru} across the range of mesh complexities considered here.
...
However, for users ultimately interested in Bayesian inference, the approximate Bayesian inference offered by \pkg{INLA}/\pkg{inlabru} is likely to be considerably faster than passing the same model from \pkg{sdmTMB}/\pkg{VAST} to \pkg{tmbstan} for full Bayesian inference.
Furthermore, although both \pkg{INLA}/\pkg{inlabru} and \pkg{sdmTMB} can use parallel processing, the PARDISO library [@pardiso-7.2c] within \pkg{INLA}/\pkg{inlabru} allows memory to be shared across cores.
...
\pkg{inlabru} is similar to \pkg{sdmTMB} in design, but differs fundamentally in that fitting is achieved via \pkg{INLA} and approximate Bayesian computation.
For some models, TMB/\pkg{sdmTMB} can be considerably faster than \pkg{INLA}/\pkg{inlabru}, but those differences depend on the SPDE mesh, model, algebra libraries, and fitting algorithm settings [Fig. \@ref(fig:timing), @osgood-zimmerman2021].
One major capability included in \pkg{sdmTMB} but not \pkg{INLA}/\pkg{inlabru} is anisotropy.
...
On the other hand, \pkg{inlabru} fits additional model classes including log Gaussian Cox processes and may be particularly usable to those already familiar with \pkg{INLA}.
...
Additional functionality in \pkg{sdmTMB} not already mentioned includes interpolating across missing time slices and forecasting, the barrier SPDE model of @bakka2019, time-varying spatiotemporal covariance parameters, and simulation from the parameter joint precision matrix.
...
The included \pkg{TMB} `.cpp` file also provides a high-quality and tested model template that can be modified to add additional features.
<!-- Not sure what kind of refs you want here?  \pkg{INLA}'s non-gaussian random fields are mentioned here @bakka2018review-->
<!-- \pkg{sdmTMB} does not currently include  -->
<!-- We may introduce explicit delta models, but many such models can already be fit with \pkg{sdmTMB} by modelling the two components separately. -->
<!-- We may introduce capability for multivariate response data, but \pkg{VAST} already focuses on such models with a \pkg{TMB} backend. -->
<!-- Furthermore, we would only add this functionality if we could maintain a straightforward interface for univariate data. -->
<!-- Line 275: I would suggest adding a MAJOR benefit of \pkg{sdmTMB} for advanced users: a tool for quickly building high-quality, bug-free, and tested model templates! This is by far my favourite feature of the brms package, as it allows me to build a pretty complex ‘base-model’ in c++/stan code before adding my own custom functionality on top of it. It saves me a bunch of time. If you don’t currently have a ‘return_TMB_code=TRUE’ option, then I would highly recommend it. -->
...
These data present statistical challenges to modelling them effectively and efficiently since appropriate models such as GLMMs with random fields are often computationally intensive and challenging to implement, interpret, and evaluate.
We hope the development of user-friendly interfaces such as \pkg{sdmTMB} opens this useful class of models to a wider audience of users.

# Acknowledgements

\pkg{sdmTMB} would not be possible without the \pkg{TMB} [@kristensen2016] and \pkg{INLA} [@rue2009; @lindgren2011; @lindgren2015] \proglang{R} packages.
...
Penalized spline support was possible thanks to \pkg{mgcv} [@wood2017a].
...
```{r child="model-description.Rmd", echo=FALSE, cache=FALSE, eval=TRUE}
```

```{r child="pcod.Rmd", echo=FALSE, cache=FALSE, eval=TRUE}
```

```{r child="owls.Rmd", echo=FALSE, cache=FALSE, eval=TRUE}
```

```{r child="inla-comparison.Rmd", echo=FALSE, cache=FALSE, eval=TRUE}
```

```{r child="timing.Rmd", echo=FALSE, cache=FALSE, eval=TRUE}
```
