...
The process component of an \pkg{sdmTMB} model can be formed by any combination of main ("fixed") effects $\boldsymbol{\beta}$ and offset $O_{\boldsymbol{s},t}$, IID (independent and identically distributed) random intercepts $\alpha_g$, spatial intercept random fields $\omega_{\boldsymbol{s}}$, spatiotemporal intercept random fields $\epsilon_{\boldsymbol{s},t}$, time-varying effects $\boldsymbol{\gamma_t}$, and spatially varying effects $\zeta_{\boldsymbol{s}}$ (Figure \ref{fig:diagram}). These components are discussed in more detail via our examples below.
...
\boldsymbol{X}^{\mathrm{svc}}_{\boldsymbol{s},t} \zeta_{\boldsymbol{s}} \right),
\end{aligned}
$$
This process component is combined with an observation error family (e.g., Gaussian, Gamma, binomial, Tweedie) and link function $f^{-1}$ (e.g., identity, log, logit) as in any generalized linear model (GLM) [@mccullagh1989].
...
$$
\begin{aligned}
y_{\boldsymbol{s}} &\sim \operatorname{Bernoulli} \left(\mu_{\boldsymbol{s}}\right),\\
\mu_{\boldsymbol{s}} &= \operatorname{logit}^{-1} \left( \boldsymbol{X}^{\mathrm{main}}_{\boldsymbol{s}} \boldsymbol{\beta} +
\omega_{\boldsymbol{s}}
\right),
\end{aligned}
$$

where $\boldsymbol{X}^{\mathrm{main}}_{\boldsymbol{s}}$ contains main effect covariates (intercept, quadratic effects of depth), $\boldsymbol{\beta}$ represents estimated main effect coefficients, and $\omega_{\boldsymbol{s}}$ represents the estimated spatial field, 

$$
\omega_{\boldsymbol{s}} \sim \mathrm{MVN}(\boldsymbol{0},\boldsymbol{\Sigma}_\omega)
$$
...
```{r pcod-predict-maps, fig.width=10, fig.asp=0.4, out.width="6.1in", fig.cap="TODO"}
...
f^{-1} \left( \boldsymbol{X}^{\mathrm{main}}_{\boldsymbol{s},t} \boldsymbol{\beta} +
O_{\boldsymbol{s},t} +
\boldsymbol{X}^{\mathrm{tvc}}_{\boldsymbol{s},t} \gamma_t +
\omega_{\boldsymbol{s}} +
\epsilon_{\boldsymbol{s},t} 
\right),
\end{aligned}
...
where $\boldsymbol{\beta}$ are coefficients associated with the main effects, $O_{\boldsymbol{s},t}$ represents the offset (here, log area swept), $\gamma_t$ represents the time-varying coefficients, $\omega_{\boldsymbol{s}}$ is the spatial field (equivalent to a spatial intercept) and $\epsilon_{\boldsymbol{s},t} $ represents spatiotemporal fields.
...
```{r dog-wvci-pred, fig.asp = 0.7, fig.cap="Example prediction components... TODO", fig.width=9, out.width="6in"}
cowplot::plot_grid(
  g_nonrf,
  g_omega,
  g_eps,
