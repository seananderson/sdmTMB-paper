...
With respect to computational speed, \pkg{sdmTMB} (and due to having the same \pkg{TMB} backend, \pkg{VAST}) were fastest up to at least 1000 mesh nodes at approximately 4--16 times faster than \proglang{R}-\pkg{INLA}/\pkg{inlabru} across the range of mesh complexities considered here.
...
The included \pkg{TMB} `.cpp` file provides a high-quality and tested model template that can be modified to add additional features.
