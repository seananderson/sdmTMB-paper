...
\begin{table}
\caption{Symbol notation, code representation (in model output or in model template code), and descriptions.}
\centering
\begin{tabular}[t]{lll}
\toprule
...
$\zeta_{\s}$ & \texttt{zeta\_s} & Spatially varying coefficient (SVC) random field at point $\s$ (knot)\\
$\zeta_{\s}^*$ & \texttt{zeta\_s\_A} & SVC random field at point $\s$ (interpolated)\\
...
$\boldsymbol{A}$ & \texttt{A} & Sparse projection matrix (from knot to data locations)\\
...
Code defined [within sdmTMB](https://github.com/pbs-assess/sdmTMB/blob/18a39eabc111e2179fa589f942c8820d87ad10df/src/utils.h#L47-L54) based on the \pkg{TMB} `dnorm()` normal density.
