...
We find that the Poisson-link delta-lognormal model \citep{thorson2018poisson} is favoured by conditional AIC.
...
Centering and scaling variables (by their mean and standard deviation here) can be especially helpful when estimating SVCs.
