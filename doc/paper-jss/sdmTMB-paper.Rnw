...
The SPDE approach has been widely adopted, especially via the \INLA\ package \citep{rue2009, lindgren2011, lindgren2015} and an implementation in \TMB\ \citep[Template Model Builder,][]{kristensen2016} that relies on \INLA\ to create input matrices \citep{thorson2015ecology, thorson2015ices, thorson2019, osgood-zimmerman2023}.  This SPDE method approximates a Matérn correlation function as arising mechanistically from local diffusion in space and/or time \citep{lindgren2011} and it results in a sparse precision matrix that permits efficient computation using existing sparse-matrix libraries \citep{rue2005gmrf}.  
...
\Rev{B2}Many features of \sdmTMB\ may be found in other software (Table~\ref{tab:functionality}), but to date no package has included all in a single package.
One common application in the field of ecology is species distribution models (SDMs), hence the package name (i.e., SDMs with \TMB: \sdmTMB), although the package is widely applicable to other fields and any geostatistical data collected continuously in space and approximated in discrete time-intervals.
...
\end{aligned}
\]
where the expected value $\mathbb{E}[.]$ of an observation $y$ at coordinates in space $\bm{s}$ is equal to mean $\mu_{\bm{s}}$.
...
Alternatively, $\omega_{\bm{s}}$ can be thought of representing spatially correlated noise arising from unmodeled processes.  % Jim: Similar to the recent Ives MEE perspectives, I prefer to discuss unmodeled stochastic processes, rather than the term "error" or "noise", so suggesting a compromise wording here. 
...
\Rev{B4a}In practice, working with the dense covariance matrix $\bm{\Sigma}$ is computationally expensive
...
\Rev{B4b}For a user of \sdmTMB, the following are the important elements to understand.
...
The objective function returns the marginal log likelihood and its gradient, integrating over random effects with the Laplace approximation \citep{kristensen2016} and efficient using sparse-matrix computation provided by \pkg{Matrix} as interface to the \pkg{Eigen} library in C++.
% Jim:  I recommend adding a citation to package Matrix ( Bates D, Maechler M, Jagan M (2023). _Matrix: Sparse and Dense Matrix Classes and Methods_. R package version 1.6-4, <https://CRAN.R-project.org/package=Matrix>.)), as central dependency of TMB, given the detailed description of the dependency stack. If really motivated, you could also add the reference for Eigen (Guennebaud G, Jacob B, et al. (2010). “Eigen v3.” http://eigen.tuxfamily.org), although it's less clear what citation to use.
...
\Rev{A19}Random effect values are returned at values that maximize the likelihood conditional on the fixed effects at their maximum marginal likelihood (i.e., plug-in or empirical Bayes estimates); however,
it is also possible to apply the epsilon estimator \citep{thorson2016bias}, which corrects for bias arising from the variance and skewness of random effects when calculating an estimator as a non-linear transformation or random effects (see Section \ref{index-bias-correct} for an example).
...
\Rev{A4}These priors or penalties are available both with maximum likelihood estimation and with MCMC sampling.
...
We can also see that the marginal AIC of the model with spatial random fields is
lower:
% JIM: Please feel free to email me, but if you're using the marginal likelihood to compute AIC then it's using marginal AIC.  I have a paper in revision @ Ecology discussing TMB calculation for conditional AIC, and happy to share or discuss further. 
...
We will do 10-fold cross validation with the folds constructed randomly. We will set the seed each time to ensure the folds are consistent.
...
The grid contains spatial covariate columns and all predictors used in the model set at values for which we want to predict.  Some covariates might be fixed at a specified value for all predictions, such that we are predicting the expected value for samples conditional on those specified values.  We call these covariates \emph{detectability} or \emph{catchability} covariates, given the model is predicting the target variable while controlling for the additional influence of these detectability covariates that was estimated in fitted data \cite{thorson2019}.  
...
% Jim:  I'm adding a sentence here about catchability/detectability covariates.  I'd be happy to discuss if y'all disagree with including this.  Obviously I'm concerned about people moving back to the fisheries-standard of not carefully considering what is a catchability vs. density covariate, and would love to preserve this as distinction that is highlighted (even if it's not explicit in the model structure itself).
...
Note that we choose to omit the spatiotemporal random fields from the binomial model component (\code{spatiotemporal = list("off", "iid")}) because initial experimentation indicated this random field was collapsing to zero indicating minimal changes in encounter spatial distribution from year to year.
...
\end{aligned}
\]
where $\rho$ represents the estimated autoregressive parameter allowing the spatial field at time $t$ to be correlated with the spatial field at time $t-1$ with deviations created by $\bm{\delta}_t$, which are themselves independent random fields each year.  This is equivalent to a separable model GMRF with precision arising from the kronecker product of the spatial precision and an AR1 temporal precision.  
%% Jim:  Feel free to drop the last sentence, but I think statisticians would wanna emphasize that this is a separable space-time formulation.
...
% Jim:  It might be worth emphasizing that this calculation of marginal effects will differ from a partial dependence plot (PDP) when depth is correlated with other variables, and that package pdp could be used to additionally compute the PDP.  Please tell me if you want me to suggest text or a code-demo, etc.
...
\Rev{A19b}We supply the grid cell area (4 km\textsuperscript{2}) to the \code{area} argument and specify \code{bias\_correct = TRUE} to enable a bias correction needed due to the non-linear transformation of the random effects \citep{thorson2016bias}.
...
Centering and scaling variables (by their mean and standard deviation here) can be especially helpful to reduce the correlation between the SVC and other random fields that are included in the model when estimating SVCs.
...
\Rev{B7}The spatially varying effect at any point in space is the combination of the main effect and SVC random effect for \code{nao}.
...
We illustrate a way of accomplishing this by simulating from uncertainty about fixed and random effects while assuming that parametric uncertainty is well approximated using a multivariate normal distribution and the joint precision matrix.  % Jim:  I found the previous wording a bit confusing, so suggested an edit.
...
\pkg{VAST} is the sole option for fitting some multivariate models; alternatively, because \pkg{VAST} focuses on multivariate delta models and fisheries applications, users fitting ``simple'' univariate spatial/spatiotemporal GLMMs in non-fisheries contexts will likely find \sdmTMB\ more straightforward.
...
\Rev{B3}Overall, we think there is value in that \sdmTMB\ unites functionality useful in many applied settings into a single package.
...
\Rev{B9a}Examples include spatial models of disease spread \citep{moraga2021}, spatial econometric models of quantities such as housing prices \citep{bivand2014}, analyzing medical imaging data such as MRI scans \citep{naseri2022}, and geophysical models of seismic waves following earthquakes \citep{zhang2015}.
\Rev{B9b}The \sdmTMB\ model is further relevant to what is commonly referred to as spatial \citep{elhorst2010, lee2010} and dynamic spatial panel data models \citep{elhorst2012} in econometrics.
...
In contrast, non-parametric approaches do not estimate spatial covariance functions [e.g., \pkg{randomForest} \citep{liaw2002}, \pkg{MaxEnt} \citep{phillips2006}, and most smooths in \pkg{mgcv} \citep{wood2017a}]. %  I think semi-parametric is defined differently in different contexts, but don't agree with including it in that claim so removing it rather than adding clarifying text. 
...
(4) the ability to specify observation-specific likelihoods to integrate different data types \citep{gruss2019multiple}, and
...
% Are y'all still planning on multivariate models?  Consider removing, or I'd be happy to discuss further?
