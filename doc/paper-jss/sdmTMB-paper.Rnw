...
Data are often collected in space or in space repeatedly over time. While such data are a rich source of information across many fields,
...
\Phi\left( \bm{s}_j, \bm{s}_k \right) =
  ...
  \left( \kappa \lVert h \rVert \right).
\]
The parameter $\sigma^2$ is the marginal variance (magnitude of the random field ``wiggles''), $\Gamma$ represents the Gamma function, $K_\nu$ represents the modified Bessel function of the second kind, and $\kappa$ represents the spatial decorrelation rate.
...
A common definition is $\sqrt{8 \nu} / \kappa$ (so if $\nu = 1$, $\mathrm{range} = \sqrt{8} / \kappa$), which has the empirically derived property of the distance at which correlation decays to $\approx 0.13$ \citep{lindgren2011} (Figure~\ref{fig:matern-range}).
...
\end{aligned}
\]
where the $\bm{X}$ represent model matrices, $\bm{\gamma_t}$ represents a vector of coefficients that are allowed to vary through time as random walks or AR(1) processes, $\zeta_{\bm{s}}$ represents spatially varying coefficients following a random field, $\alpha_g$ represents IID random intercepts by group $g$ ($\alpha_g \sim \operatorname{Normal} \left(0, \sigma_\alpha^2 \right)$), $O_{\bm{s},t}$ represents an offset variable \citep[][p.~206]{mccullagh1989} (e.g., log sampling effort), and $\omega_{\bm{s}}$ and $\epsilon_{\bm{s},t}$ represent spatial and spatiotemporal intercept random fields as before (Figure~\ref{fig:diagram}).
...
% (\code{browseVignettes("sdmTMB")} or see the
...
Observation distributions and links are specified with standard \R\ family functions (e.g., \fct{binomial()}) plus several \sdmTMB-specific families not available in the \R\ stats library (e.g., \fct{nbinom2()}, \fct{delta\_lognormal()}, see \fct{?sdmTMB::Families}).
...
Supported functionality includes bivarate smoothers \fct{s(x, y)},
smoothers varying by continuous or categorical variables \fct{s(x1, by = x2)},
cyclical smoothers \fct{s(x, bs = "cc")},
and smoothers with specified basis dimensions \fct{s(x, k = 4)} \citep{wood2017a}.
...
The negative marginal log likelihood is minimized via the non-linear optimization routine \fct{stats::nlminb()} \citep{gay1990} in \R\ with additional optimization carried out via a Newton optimization step to find the Hessian using \fct{stats::optimHess()} \citep{gay1990, r2021}.
...
Others standard S3 methods are also available such as \fct{fixef()}, \fct{confint()}, and \fct{vcov()}.

Prediction on fitted or new data is accomplished with \fct{predict()} (\fct{?predict.sdmTMB}).
The \fct{predict.sdmTMB()} method is flexible and includes the option to specify a new data frame (\fct{newdata}),
...
Following other recent SPDE implementations in \TMB\ \citep{breivik2021, osgood-zimmerman2021}, penalized complexity (PC) priors \citep{simpson2017, fuglstad2019} (\code{?pc\_matern}) can constrain the spatial range and variance parameters.
...
% minimized via the non-linear optimization routine \code{stats::nlminb()} in
...
% \citep[AIC,][]{akaike1974} can be calculated with \fct{AIC()}, although AIC
...
% The SPDE precision matrix calculations themselves are performed by functions within \TMB\ \citep{kristensen2016} (\code{R\_inla} namespace).
% Data structures required to implement penalized smoothers are constructed with \fct{smooth2random()} from \pkg{mgcv} \citep{wood2017a}, similarly to \pkg{gamm4} \citep{wood2020} and \pkg{brms} \citep{brms}.
...
We can also specify additional arguments, in this case passed to \fct{fmesher::fm\_mesh\_2d\_inla()}: a maximum triangle edge length of 10 km and 40 km for the inner and outer mesh, and an offset width of 10 km and 40 km for the inner and outer meshes borders.
% Alternatively, we could have created any mesh via \INLA\ or \pkg{fmesher} and supplied it to the \code{mesh} argument in \fct{make\_mesh()}.
...
<<dog-binomial-mesh2, results='hide', message=FALSE, warning=FALSE, echo=TRUE, fig.cap="SPDE mesh (lines) combined with the trawl survey observations (points). The locations where lines intersect are referred to as ``vertices''. Finer meshes will be slower to fit but generally increase the accuracy of the SPDE approximation. A greater degree of control over the mesh construction can be achieved by using \\pkg{fmesher} or \\proglang{R}-\\pkg{INLA} directly and supplying the resulting object to \\code{make\\_mesh()}.", fig.width=4.5, fig.asp=1, out.width="3in">>=
...
We will use the \fct{update()} S3 method to refit the model while updating any specified arguments:
...
We can test for spatial autocorrelation with a Moran's I test or with a visual inspection of the residuals calculated from the \fct{residuals()} S3 method.
...
We can do additional model comparison with cross validation via \fct{sdmTMB\_cv()}.
We will do 10-fold cross validation with the folds constructed randomly.
Using the \code{fold\_ids} argument, we could supply our own folds and conduct spatially blocked cross validation. If we set a parallel plan with the \pkg{future} package \citep{bengtsson2021future}, our folds will be fit in parallel.
...
We can plot these with \fct{geom\_raster()} from the \pkg{ggplot2} package (Figure~\ref{fig:pcod-predict-maps}).
...
Note that we choose to omit the spatiotemporal random fields from the binomial model component (\code{spatiotemporal = list("off", "iid")}) because initial fits indicated this random field was collapsing to zero indicating minimal changes in encounter spatial distribution from year to year.
...
Starting with the binomial component, we have output from the smoother, which includes a linear component (\code{slog\_depth}) and the standard deviation on the smoother weights (\code{sds(log\_depth)}).
...
Our prediction data frame is similar to the binomial spatial model, but includes columns for the two delta model components (labelled with suffixes \code{1} and \code{2}) and adds an \code{epsilon\_st} column for spatiotemporal random effects.
We can easily generate plots from this data frame using \pkg{ggplot2} code with \fct{geom\_raster()} similar to our spatial example with Pacific Cod (Figure~\ref{fig:dog-wcvi-pred}).
...
The spatially varying coefficient effect at any point in space is the combination of the main effect and spatially varying random effect for \code{nao}.
Mean estimates of the spatially varying random effect are available in the output of \fct{predict.sdmTMB()} in a column starting with \code{zeta\_s} (here, \code{zeta\_s\_nao}).
...
By default, \code{nsim > 0} will return a matrix of draws from the overall prediction.
Here, we instead specify that we want to return draws from the \code{zeta\_s} ($\zeta_{\bm{s}}$) random field, which is the spatially varying random field.
This returns a matrix where each row matches a row of \code{newdata} and each column is a simulation draw.
We then use the function \fct{spread\_sims()} to draw 200 simulations from the parameters themselves, combine the main effect draws (\code{sims\$nao}) with the random field values \code{zeta\_s}, and calculate the median for each column of data so we can plot them.
...
\pkg{inlabru} used the empirical Bayes integration strategy and Gaussian approximation with \code{bru\_max\_iter = 1}, and the \fct{like()} formulation.
\pkg{mgcv} used \fct{bam()}, \code{method = `fREML'}, and discretized covariates \citep{miller2019}.
...
% The Matérn standard deviations are estimated separately for the various fields and the range---the distance at which spatial correlation decays to \(\sim 0.13\) \citep{lindgren2015}---can be shared among random fields or estimated separately (\code{share\_range} argument).
% TODO: figure out where to put this paragraph
...
% The included \TMB\ \code{.cpp} file is flexible and provides a tested model template that can be modified to add additional features.
