...
The grid contains spatial covariate columns and all predictors used in the model set at values for which we want to predict.  Some covariates might be fixed at a specified value for all predictions, such that we are predicting the expected value for samples conditional on those specified values.
We call these covariates \emph{detectability} or \emph{catchability} covariates, given the model is predicting the target variable while controlling for the additional influence of these detectability covariates that was estimated in fitted data \cite{thorson2019}.
...
This is equivalent to a separable model GMRF with precision arising from the kronecker product of the spatial precision and an AR1 temporal precision.
...
We illustrate a way of accomplishing this by simulating from uncertainty about fixed and random effects while assuming that parametric uncertainty is well approximated using a multivariate normal distribution and the joint precision matrix.
...
% TODO: add multivariate responses to mgcv?
...
(1) the ability to specify observation-specific likelihoods to integrate different data types \citep{gruss2019multiple},
(2) inclusion of a dispersion formula similar to \pkg{glmmTMB},
(3) basic multivariate models to allow for simultaneous modelling of data from multiple groups,
and
(4) integration with the \pkg{RTMB} \citep{rtmb} package so that the model code base will be written in \R\ rather than \proglang{C++},
...
% TODO: Jim: Are y'all still planning on multivariate models?  Consider removing, or I'd be happy to discuss further?
% SA: added 'basic', which should be true (already in a branch... but unlikely to go too far with this
