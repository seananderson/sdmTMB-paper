...
We note that the specific p-value is dependent on the seed due to the randomization in the randomized quantile residuals.
}
...
\pkg{spaMM} scaled well to large datasets with high mesh resolutions (Figure~\ref{fig:timing}c), but can only fit spatial and not spatiotemporal models.
...
The models were fit on a 2023 16 inch M2 MacBook Pro with an Apple M2 Pro 10-core CPU and 32 GB of RAM in R 4.2.2 and the default \pkg{BLAS} \citep{blas} library packaged with \R \citep{r2024}.
