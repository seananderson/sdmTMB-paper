...
value of zero that is added to the expectation in link space.

The vector of $\omega_{\bm{s}}$ ($\bm{\omega}$) is assumed drawn from
a multivariate normal distribution with covariance matrix
$\boldsymbol{\Sigma}_\omega$ constrained by some covariance function that
defines the rate that spatial covariance decays with distance. Various
covariance functions are possible, but a popular and flexible function is the
Mat\'ern (REF) (FIGURE TODO). If we define $\lVert h \rVert$ as the Euclidean
...
as $\textrm{range} = \sqrt{8} / \kappa$ and (2) marginal standard deviation as
$\sigma = 1 / \sqrt{4 \pi \exp \left(2 \log(\tau) + 2 \log(\kappa) \right) }$.
The range is the distance that correlation decays to $\approx 0.13$.
