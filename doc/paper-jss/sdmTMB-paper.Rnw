...
The Mat\'ern function describes the covariance $\Phi \left( s_j, s_k \right)$ between spatial locations $s_j$ and $s_k$. If we define $\lVert d \rVert$ as the Euclidean distance between $s_j$ and $s_k$, we can define the covariance as
\[
\Phi\left( s_j, s_k \right) = \frac{\tau^2}{2^{\nu - 1}\Gamma(\nu)}
\left(\kappa \lVert d \rVert \right)^\nu K_\nu \left( \kappa \lVert d \rVert \right),
\]
where $\tau^2$ controls the spatial variance, $\Gamma$ represents the Gamma
function, $K_\nu$ represents the modified Bessel function of the second kind,
and $\kappa$ represents the spatial decorrelation rate. The parameter $\nu$
controls the smoothness of the covariance function, which is fixed at $\nu = 1$, to take advantage of the SPDE \citep{lindgren2011}.
Internally, the parameters $\kappa$ and $\tau$ are converted to correlation range as $\textrm{range} = \sqrt{8} / \kappa$ and to a marginal standard deviation as $\sigma = 1 / \sqrt{4 \pi \exp \left(2 \log(\tau) + 2 \log(\kappa) \right) }$.
...
\]
Here, we add a linear transformation matrix $\bm{H}$ with two estimated parameters governing the major axis direction of geometric anisotropy and the ratio of the major and minor axes \citep{lindgren2011, thorson2015ices, thorson2018comp}. Again, the smoothness parameter $\nu$ is fixed at 1.
...
We can extend our model to accommodate spatiotemporal data by adding Gaussian random fields for each time step $t$, $\epsilon_{\bm{s},t}$
...
\subsection{Adding additional model components}

In practice, the above models can become considerably more complex by, for example, structuring the spatiotemporal random fields as autoregressive, letting coefficients vary though time, letting coefficients vary through space as random fields (i.e., SVC REF), or including offsets (REF). All these components are additive in link space. Adopting the notation ``main'' for main effects, ``tvc'' for time-varying coefficients, and ``svc'' for spatially varying coefficients, a maximally complex model could be written as
\[
\begin{aligned}
\mu_{\bm{s},t} &=
...
\end{aligned}
\]
where the $\bm{X}$ represent model matrices, $\bm{\gamma_t}$ represents a vector of coefficients that are allowed to vary through time as random walks or AR(1) processes, $\zeta_{\bm{s}}$ represents spatially varying coefficients following a GRF, $\alpha_g$ represents IID random intercepts by group $g$ ($\alpha_g \sim \operatorname{Normal} \left(0, \sigma_\alpha^2 \right)$, $O$ represents an offset variable (e.g., log sampling effort), and $\omega_{\bm{s}}$ and $\epsilon_{\bm{s},t}$ represent spatial and spatiotemporal intercept random fields as before. We demonstrate all of these model components in the examples later in this paper.

\subsection{Delta models}

So far we have described models with one linear predictor and one family. Frequently, observations are better represented with two-part delta or hurdle models, which include linear predictors and observation distributions for two processes: zero vs.\ non-zero values and positive values. Here, we describe two types of delta models. We drop the space and time subscripts for simplicity and use $\bm{X_1} \bm{\beta_1}$ and $\bm{X_2} \bm{\beta_2}$ to refer to generic model matrices and parameter vectors for the first and second model components.
...
The prediction for a new data point $y$ is then $p \cdot r$.

\subsubsection{Poisson-link delta models}

An additional delta-model is possible that has several advantages over the logit-log delta model \citep{thorson2018poisson}. The primary advantage is that both linear components use a log link so the effects are additive in log space and can be combined. In this case, there are two linear predictors, but they represent a theoretical group number $n$ and a theoretical weight per group $w$ \citep{thorson2018poisson}.
\[
\begin{aligned}
...
\end{aligned}
\]
Note that the first linear predictor has a log link, \emph{not} a logit link and the linear predictor predicts group number $n$, not positive observation probability $p$. These get transformed \citep{thorson2018poisson} via
\[
\begin{aligned}
p &= 1 - \exp(- \exp( \log n)),\\
r &= \frac{n w}{p}.
\end{aligned}
\]
The first part is the complementary-log-log inverse link \citep[][p.~31]{mccullagh1989} but the group number $n$ also enters into the expected positive rate $r$. These probabilities $p$ and positive rates $r$ are then entered into a Bernoulli and positive observation distribution as before
\[
\begin{aligned}
\mathrm{I}(y > 0) &\sim \operatorname{Bernoulli} \left( p \right),\\
y | y > 0 &\sim \operatorname{Positive-distribution} \left( r, \phi \right).
