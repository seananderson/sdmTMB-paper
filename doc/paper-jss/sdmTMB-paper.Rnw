...
Because both productivity and the choice of wintering location could be influenced by climate, we modelled a spatially varying effect of annual mean NAO index on early winter abundance across the southern boundary of their winter distribution.
Annual mean NAO captures the preceding winter's conditions, combined with breeding season and early winter climate.

To do this we use counts of Snowy Owls observed by the annual Christmas Bird Counts \citep{cbc} from all locations where they have been recorded and for which there was at least 3 counts conducted between the years 1979 and 2020.
Our data are read in from supplementary data and also contain columns for spatial coordinates (in an Albers projection for North America and divided by 100000 to give units of 100 km), year, year as a factor, and the annual NAO value:
...
select(snow, X, Y, year, year_f, nao, count) |> head(n = 5)
@

\subsection{Fitting the model}

We will fit counts using a negative binomial \citep[NB2,][]{hilbe2011} distribution, random intercepts for year, spatial and spatiotemporal random fields, and a spatially varying coefficient associated with the NAO.
Centring and scaling variables is especially helpful when estimating spatially varying coefficients.
NAO is an index so it can be left as is, but including it as a main effect as well ensures that the spatial coefficients will have a mean of 0.
...
But given that this is only part of the effect of NAO that we include in our model, how do we assess if this variable has a meaningful effect in some locations, even if not overall?

\subsection{Extracting the spatially varying coefficient effects with uncertainty}
...
Mean estimates of the spatially varying random effect are available in the output of \fct{predict.sdmTMB} in a column starting with \code{zeta\_s} (in this case would appear as \code{zeta\_s\_nao}).
...
Here, we instead specify that we want to return draws for the \code{zeta\_s} ($\zeta_{\bm{s}}$) random field, which is the spatially varying random field.
...
Because the simulations are stored in different dimensions for these two functions, the random field draws must be transposed \code{t()} before combining the main effect draws (\code{sims\$nao}) with the random field values \code{zeta\_s}.
...
For this example we use thresholds of 0.1 and 0.9 representing CIs of 80\%.
...
We can make a basic plot using the following code. A more elaborate version including separate panels for each of the CIs is shown in Figure~\ref{fig:owl-plot-fancy}a.
...
However, if we recalculated the CI at 95\% level the full range of estimates for the lower CI would be below 1 meaning that at an alpha level of 0.05, we cannot conclude that the effect of NAO at an annual scale differs significantly from 1.
...
<<owl-plot-fancy, echo=FALSE, eval=TRUE, fig.cap="Spatially varying coefficients for effect of mean annual NAO (North Atlantic Oscillation) on counts of Snowy Owls observed on annual Christmas Bird Counts from 1979--2020 in Canada and the US. The effect is multiplicative on owl count per NAO unit. In the west, the lower bound of values overlaps 1 implying no effect, whereas in the southeast the effect becomes positive. Point size is scaled to the mean counts in each location.", fig.width=5, out.width="4in", fig.asp=1, fig.pos="htbp", fig.align='center'>>=
