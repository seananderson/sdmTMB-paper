...
The primary advantage is that both linear predictors use a log link so the linear predictors can be added in link space. 
...
The package gathers functionality not found combined in other packages that is particularly useful to species distribution modelling, but is applicable far beyond ecology.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=3.8in]{flowchart}
\caption[]{
TODO
}\label{fig:flowchart}
\end{figure}
...
Families are specified with standard R family functions (e.g., \texttt{binomial()}) plus several \sdmTMB-specific families for family's not available in the \R\ stats library (e.g., \texttt{nbinom2()}, \texttt{delta\_lognormal()}).
...
If a model will be passed to \pkg{tmbstan}, a \texttt{bayesian} logical argument is available to enable Jacobian adjustments for change of variables (priors applied to parameters that are internally transformed) (REF).
...
The random fields in \pkg{sdmTMB} are structured as MVN constrained by a Matérn covariance function \citep{matern1960}. 
