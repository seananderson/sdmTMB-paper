...
SVC models \citep{hastie1993} are a class of models in which coefficients are allowed to vary spatially constrained by some smooth function.
...
select(snow, X, Y, year, year_f, nao, count) |> head(n = 6)
@
