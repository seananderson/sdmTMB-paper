...
\end{aligned}
\]
where the expected value $\mathbb{E}[.]$ of an observation $y$ at coordinates in space $\bm{s}$ is equal to mean $\mu_s$. That mean is the result of an inverse link function $g^{-1}$ applied to a linear predictor $\eta$. In this case, that linear predictor is the combination of a model matrix $\bm{X}_{\bm{s}}$ multiplied by a vector of coefficients $\bm{\beta}$ and a value from a spatial random field $\omega_{\bm{s}}$. This spatial random field represents the effect of latent spatial variables that are not otherwise accounted for in the model. Alternatively, $\omega_{\bm{s}}$ can be thought of representing spatially correlated noise. More simply, the vector of $\omega_{\bm{s}}$ ($\bm{\omega}$) represents a ``wiggly'' surface with an expected value of zero that is added to the linear predictor in link space (e.g., Figure~\ref{fig:matern-range}ab).
...
\Phi\left( s_j, s_k \right) =
  ...
  \left( \kappa \lVert h \rVert \bm{H} \right).
\]
Here, we add a linear transformation matrix $\bm{H}$ with two estimated parameters governing the major axis direction of geometric anisotropy and the ratio of the major and minor axes \citep{lindgren2011, thorson2015ices, thorson2018comp}.
...
First, the SPDE approach links Mat\'ern Gaussian random fields (GRFs) to Gaussian \emph{Markov} random fields (GMRFs) in such a way that a GMRF can be a good approximation to a GRF \citep{lindgren2011}.
...
The properties of this mesh (e.g., its resolution and how far it extends beyond the data) affect the accuracy of the SPDE approach (and involve a tradeoff between computational burden and accuracy) \citep{lindgren2011}.
Third, the Mat\'ern precision matrix calculations involve estimating the decorrelation rate parameter ($\kappa$), a parameter that scales the precision matrix ($\tau$) and three sparse matrices associated with the mesh ($\bm{C}$, $\bm{G}_1$, and $\bm{G}_2$) \citep{lindgren2011},
\[
\bm{Q} = \tau^2 \left(\kappa^4\bm{C} + 2\kappa^2\bm{G}_1 + \bm{G}_2 \right).
\]
Based on an estimate of $\tau$, we can calculate the marginal variance of the Mat\'ern random field as $\sigma^2 = \left (4 \pi \tau^2 \kappa^2 \right)^{-1}$.

%GMRF
%is an SPDE whose solution exhibits the desired covariance structure and associated precision matrix.
%The approach outlined in \citet{lindgren2011} streamlines the computation of Q and he demonstrates that
%Can then take advantage of methods for rapid precision matrix computation \citep{rue2005}

%The SPDE approach offers an efficient means to estimate Q without the typical computational overhead. It revolves around solving an equation known as the SPDE. The solutions to this equation are stochastic processes, and their covariance structure is tailored to meet the specifications outlined by the SPDE. To employ the SPDE approach effectively for f, the goal is to identify an SPDE whose solutions exhibit the desired covariance structure and implied precision matrix. In the work of Lindgren et al. (2011), a practical solution to the SPDE is presented by representing f as a combination of basis functions multiplied by coefficients. This approach significantly streamlines the computation of Q. Moreover, Lindgren et al. (2011) demonstrate that these coefficients form a Gaussian Markov random field, benefiting from existing methods for swift precision matrix computation (Rue and Held 2005). These computations pave the way for rapid model fitting using integrated nested Laplace approximations (INLA; Rue et al. 2009).


%The SPDE approach links

%A user should know that...
%A GMRF has the advantage that it can be fit modelling the precision matrix, which is computationally much more efficient than working with the covariance matrix.

%Because the parameters and input matrices are revelant to understanding sdmTMB internals, we note that the precision

%a way to approximate Q
%SPDE = an equation to be solved


%We introduce the SPDE approach to working with the precision matrix in Section \ref{sec:software}, but for now, note that we can instead represent our spatial random field as
%\[
...
In practice, the above models can become considerably more complex by, for example, letting coefficients vary though time, letting coefficients vary through space \citep{hastie1993} as random fields, or adding random intercepts by grouping factors. All these components are additive in link space. Adopting the notation ``main'' for main effects, ``tvc'' for time-varying coefficients, and ``svc'' for spatially varying coefficients, a more complex model could be written as
\[
\begin{aligned}
\mu_{\bm{s},t} &=
...
An additional delta-model is possible that has several advantages over the logit-log delta model \citep{thorson2018poisson}. The primary advantage is that both linear predictors use a log link so the linear predictors can be added in link space. In these models, the linear predictors represent a theoretical group number $n$ and a theoretical weight (e.g., mass) per group $w$ \citep{thorson2018poisson},
\[
\begin{aligned}
...
\end{aligned}
\]
Note that the first linear predictor has a log link, \emph{not} a logit link, and the linear predictor predicts group number $n$, \emph{not} positive observation probability $p$. These get transformed \citep{thorson2018poisson} via
\[
\begin{aligned}
p &= 1 - \exp(- \exp( \log n)),\\
r &= \frac{n w}{p}.
\end{aligned}
\]
The first part is the complementary-log-log inverse link \citep[][p.~31]{mccullagh1989} (which has its roots in a Poisson process) but the group number $n$ also enters into the expected positive rate $r$. These probabilities $p$ and positive rates $r$ are then entered into a Bernoulli and positive observation likelihood as before
\[
\begin{aligned}
\mathrm{I}(y > 0) &\sim \operatorname{Bernoulli} \left( p \right),\\
y | y > 0 &\sim \operatorname{Positive-distribution} \left( r, \phi \right).
...
The design goal of \sdmTMB\ was to make...
...
\sdmTMB\ relies on several well-established \R\ packages (FIG?).
\sdmTMB\ uses \pkg{fmesher} and \INLA\ to generate input matrices for SPDE calculations \citep[$\bm{C}$, $\bm{G}$, and $\bm{K}$ in][]{lindgren2011}.
The SPDE calculations themselves are performed by functions within \pkg{TMB} (\texttt{R\_inla} namespace).
Data structures required to implement penalized smoothers are constructed with \pkg{mgcv}'s \texttt{smooth2random()} function similarly to \pkg{gamm4} and \pkg{brms}.
...
After rapid model exploration with maximum likelihood, one can optionally pass an \sdmTMB\ model to the \R\ package \pkg{rstan} \citep{carpenter2017, rstan} via \pkg{tmbstan} \citep{monnahan2018} to estimate the joint posterior distribution for Bayesian inference.
...
\end{aligned}
\]
where $\rho$ represents the estimated autoregressive parameter allowing the spatial field at time $t$ to be correlated with the spatial field at time $t-1$ with deviations created by $\bm{\delta}_t$, which are themselves independent random fields by year.
