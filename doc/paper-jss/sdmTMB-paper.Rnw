...
The negative marginal log likelihood ivs minimized via the non-linear optimization routine \fct{stats::nlminb} \citep{gay1990} in \R\ with additional optimization carried out via a Newton optimization step to find the Hessian using \Rev{A6} \fct{stats::optimHess} \citep{gay1990, r2021}.
...
Other \Rev{A7} standard methods are also available such as \fct{fixef}, \fct{confint}, and \fct{vcov}.
...
\Rev{A3}We set \code{type = "mle-mvn"} to denote setting fixed effects at their maximum likelihood estimates but taking a single draw from the approximate (multivariate normal) distribution of the random effects \citep{waagepetersen2006, thygesen2017a}.
