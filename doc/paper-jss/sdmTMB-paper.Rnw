...
the SPDE approach to working with the precision matrix in the software section,
...
to using the \pkg{fmesher} \citep{fmesher} package to eliminate this dependency.
...
between mesh vertices in the units of \texttt{X} and \texttt{Y} (km). We can
also specify additional arguments, in this case passed to
\texttt{fmesher::fm\_mesh\_2d\_inla()}: a maximum triangle edge length of 20 km
and 40 km for the inner and outer mesh, and an offset border width of 5 km and
40 km for the inner and outer meshes. Alternatively, we could have created any
mesh via \proglang{R}-\pkg{INLA} or \pkg{fmesher} and supply it to the
\texttt{mesh} argument in \texttt{make\_mesh()}. We can inspect mesh object with
the associated plotting method (Figure~\ref{fig:dog-binomial-mesh2}). Our mesh
has 238 (\texttt{mesh\_pcod\$mesh\$n}) vertices. Guidelines for what makes
a good mesh are described in TODO.
