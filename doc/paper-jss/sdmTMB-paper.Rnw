...
We hope \sdmTMB will help open this useful class of models to a wider field of geostatistical analysts.
}
...
Data are often collected at particular locations in space or in space repeatedly over time. While such data are a rich source of information across many fields,
...
\Rev{B2}Many individual features of \sdmTMB\ may be found in other software (Table~\ref{tab:functionality}), but to date this full suite of useful features has not been integrated into a single package.
...
The objective function returns the marginal log likelihood and its gradient, integrating over random effects with the Laplace approximation \citep{kristensen2016} and efficient using sparse-matrix computation provided by \pkg{Matrix} as interface to the \pkg{Eigen} library in \proglang{C++}.
...
The negative marginal log likelihood ivs minimized via the non-linear optimization routine \fct{stats::nlminb} \citep{gay1990} in \R\ with additional optimization carried out via a Newton optimization step to find the Hessian using \fct{stats::optimHess} \citep{gay1990, r2021}.
...
it is also possible to apply an ``epsilon'' estimator \citep{thorson2016bias}, which corrects for bias arising from the variance and skewness caused by non-linear transformations of random effects (see Section \ref{index-bias-correct} for an example).
...
The default is a form of randomized quantile residuals \citep{dunn1996}.
...
Users willing to replace the default \R\ \pkg{BLAS} \citep{blas}
% and \pkg{LAPACK} \citep{lapack}
library
with an optimized version \citep[e.g., \pkg{openBLAS};][]{openblas}
can expect about an order of magnitude increase in model fitting speed.
Suggestions are included in the package README.
...
We set \code{type = "mle-mvn"} to denote setting fixed effects at their maximum likelihood estimates but taking a single draw from the approximate (multivariate normal) distribution of the random effects \citep{waagepetersen2006, thygesen2017a}.
...
We see strong evidence for spatial autocorrelation for the model without a random field (p $<$ 0.01) but a lack of evidence for the model with a random field (p $>$ 0.01) suggesting that residual spatial autocorrelation is alleviated by including the random field.
We note that the specific p-value is dependent on the seed due to the randomization in the randomized quantile residuals \citep{dunn1996}.
...
Indeed, the log likelihood of left-out data is considerably higher for the model that includes random fields indicating better out-of-sample predictions:
...
In practice, we would repeat this procedure several times to ensure the rank order is not sensitive to the randomly chosen folds and, if it was, consider averaging across multiple folds or increasing the number of folds.
...
Note that we need to add an extra Newton optimization step to the delta-lognormal model to achieve suitable convergence (discovered via a call to \code{sanity(fit\_dl)}).
...
We next test two additional model formulations: making the spatial correlation isotropic (the default) instead of anisotropic, and structuring the positive model component spatiotemporal random fields as AR(1) to allow spatiotemporal patterns to partially persist from year to year.
...
<<dog-residuals, fig.cap="Simulation-based randomized quantile residuals from the \\pkg{DHARMa} package. The statistical test is a two-sided Kolmogorov-Smirnov test of the null hypothesis that the residuals are drawn from a uniform(0, 1) distribution.", fig.asp=0.68, echo=TRUE>>=
set.seed(1)
s <- simulate(fit, nsim = 500, type = "mle-mvn")
dharma_residuals(s, fit)
@

The quantile-quantile plot suggests that under the model assumptions, the (transformed) residuals are consistent with an independent uniform(0, 1) distribution.
...
% \input{../../../sdmTMB-paper-response/response}
