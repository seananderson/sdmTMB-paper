...
Here, we introduce the \R\ package \sdmTMB, which extends the flexible interface familiar to users of \pkg{lme4}, \pkg{glmmTMB}, and \pkg{mgcv} to include spatial and spatiotemporal latent Gaussian Markov random fields using an SPDE-(stochastic partial differential equation) based approach.
...
Our test was restricted to one core and default \R\ algebra libraries.
...
A subset of features to be added include (1) multivariate models to allow for simultaneous modelling of data from multiple groups, (2) integration with the \pkg{RTMB} \citep{rtmb} package so that the model code base will be written in \R\ rather than \proglang{C++}, (3) inclusion of a dispersion formula similar to \pkg{glmmTMB},
(4) the ability to specify observation-level likelihoods to integrate different data types (e.g. REF),
(5) conditional autoregressive (CAR) models as an alternative to random fields for large datasets that are already areal in nature, and
(6) improved integration with existing visualization methods such as \pkg{ggeffects} \citep{ggeffects}.
