...
The negative marginal log likelihood is minimized via the non-linear optimization routine \fct{stats::nlminb} \citep{gay1990} in \R\ with additional optimization carried out via a Newton optimization step to find the Hessian using \fct{statsv::optimHess} \citep{gay1990, r2021}.
% A19
...
bias-corrected versions of derived quantities based on non-linear transformations of random effects are possible \citep{thorson2016bias} (see Section \ref{index-bias-correct} for an example).
Standard errors on all parameters and derived quantities---including those involving random effects---are calculated using the generalized delta method \citep{kristkristensen2016, zheng2021}.
...
The overall predictions combining fixed and random effects are plug-in predictors combining
...
SVC models are a class of models in which coefficients are allowed to vary spatially constrained by some smooth function \citep{hastie1993}.
...
Our data are read from supplementary data and also contain columns for spatial coordinates (in an Albers projection for North America and divided by 100000 to give units of 100 km), year, year as a factor, the annual NAO value, and own count:
...
Centering and scaling variables (say by there mean and standard deviation) is especially helpful when estimating SVCs.
Here, NAO is an index with a mean near zero and an SD not too far from 1, so we will leave it as is.
However, we also include NAO as a main effect since the SVC random field is assumed to have a mean of zero.
...
However, if we recalculated the CI at 95\% level the full range of estimates for the lower CI would be below 1 meaning that at an alpha level of 0.2, we cannot conclude that the effect of NAO at an annual scale differs significantly from 1.
...
However, \INLA/\pkg{inlabru} and \pkg{spaMM} were less affected by mesh resolution than \sdmTMB and \pkg{mgcv} was most affected by mesh resolution (Figure~\ref{fig:timing}).
For users ultimately interested in Bayesian inference, the approximate Bayesian inference offered by \INLA/\pkg{inlabru} is likely to be considerably faster than passing the same model from \sdmTMB/\pkg{VAST} to \pkg{tmbstan} for full Bayesian inference.
...
Additional functionality in \sdmTMB\ not already mentioned includes interpolating across missing time slices and forecasting, the barrier SPDE model \citep{bakka2019}, time-varying spatiotemporal covariance parameters \citep{ward2022}, and integration with visualization tools such as \pkg{ggeffects} \citep{ggeffects}.
...
(4) the ability to specify observation-level likelihoods to integrate different data types, and
