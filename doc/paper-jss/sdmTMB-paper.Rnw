...
and that the random field Matérn range parameter does not look unreasonably large ($<$ 1.5 times the largest distance from a bounding box around the observations).
...
We can also see that the conditional AIC of the model with spatial random fields is
lower:
...
Indeed, the log likelihood of left-out data is considerably larger for the model that includes random fields indicating better out-of-sample predictions:
...
In practice, we would want to repeat this procedure several times to ensure the rank order is not sensitive to the randomly chosen folds and, if it was, consider averaging across multiple folds or increasing the number of folds.
...
\end{aligned}
\]
where \(\bm{\beta}\) are coefficients associated with the main effects, \(O_{\bm{s},t}\) represents the offset (here, log area swept), \(\gamma_t\) represents the time-varying coefficients, \(\omega_{\bm{s}}\) is the spatial field (representing constant latent spatial effects) and \(\epsilon_{\bm{s},t}\) (representing latent spatial effects that vary by year).
...
Our data are read in from supplementary data and contain columns for spatial coordinates (in an Albers projection for North America and divided by 100000 to give units of 100 km), year, year as a factor, and the annual NAO value:
<<owl-data>>=
...
We will fit counts observed in North America during annual Christmas Bird Counts \citep{cbc} using a negative binomial \citep[NB2,][]{hilbe2011} distribution, random intercepts for year, spatial and spatiotemporal random fields, and a spatially varying coefficient associated with the NAO.
