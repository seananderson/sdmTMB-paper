...
Because both productivity and the choice of wintering location could be influenced by climate, we modelled a spatially varying effect of annual mean NAO index on the end of year's abundance across the southern boundary of their winter distribution.
...
select(snow, X, Y, year, year_f, nao, count)
@
...
We then use the function \fct{spread\_sims} to draw 200 simulations from the parameters themselves, combine the main effect draws (\code{sims\$nao}) with the random field values \code{zeta\_s}, and calculate the median, and upper and lower quantiles for each column of data so we can plot them.
For this example we use thresholds of 0.1 and 0.9 representing confidence invervals (CI) of 80\%.
...
We can make a basic plot using the following code. A more elaborate version including the CIs is shown in Figure~\ref{fig:owl-plot-fancy}a.
...
<<owl-plot-fancy, echo=FALSE, eval=TRUE, fig.cap="Spatially varying coefficients for effect of mean annual NAO (North Atlantic Oscillation) on counts of Snowy Owls observed on annual Christmas Bird Counts from 1979--2020 in Canada and the US. The effect is multiplicative on owl count per NAO unit. In the west, the lower bound of values overlaps 1 implying no effect, whereas in the southeast the effect becomes positive. Point size is scaled to the mean counts in each location.", fig.width=5, out.width="5.25in", fig.asp=1, fig.pos="htbp", fig.align='center'>>=
nao_effect_df <- select(snow, X, Y, count, nao_effect, nao_effect_lwr, nao_effect_upr) |>
  group_by(X,Y) |> mutate(mean_count = mean(count, na.rm = TRUE)) |> select(-count) |>
  distinct() |>
...
snow_g1 <- ggplot(data = nao_effect_df) +
  ...
  geom_point(aes(X, Y, colour = nao_effect, size = mean_count), alpha = 0.5) +
  geom_sf(data = coast, colour = "gray50", fill = NA) +
...
snow_g2 <- ggplot(data = nao_effect_df) +
  geom_sf(data = land, fill = "white", colour = "white",lwd = 0.35) +
  geom_sf(data = lakes, colour= "gray23", fill = "grey90", lwd = 0.35) +
  geom_point(aes(X, Y, colour = nao_effect_lwr, size = mean_count), alpha = 0.5) +
...
snow_g3 <- ggplot(data = nao_effect_df) +
  geom_sf(data = land, fill = "white", colour = "white",lwd = 0.35) +
  geom_sf(data = lakes, colour= "gray23", fill = "grey90", lwd = 0.35) +
  geom_point(aes(X, Y, colour = nao_effect_upr, size = mean_count), alpha = 0.5) +
  geom_sf(data = coast, colour = "gray50") +
  geom_sf(data = lakes, colour = "gray50", fill = NA) +
  coord_sf(
    xlim = c(min(snow2$X), max(snow2$X)),
