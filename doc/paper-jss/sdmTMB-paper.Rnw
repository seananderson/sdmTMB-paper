...
Others standard methods are also available such as \fct{fixef}, \fct{confint}, and \fct{vcov}.
...
We can test for spatial autocorrelation with a Moran's I test or with a visual inspection of the residuals calculated from the \fct{residuals} method.
