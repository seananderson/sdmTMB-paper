...
depart from the conventional delta models in treating the probability of
occurrence at a given location ($p_i$) as a Poisson process rather than
a Bernoulli one, $p_i = 1 - exp(a_i * n_i)$, where $n_i$ is the total number of
individuals available to be encountered, and $a_i$ is a measure of sampling
effort \citep{thorson2018poisson}. Note that we choose to omit the
