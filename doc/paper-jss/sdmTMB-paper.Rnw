...
In practice, inversion of the covariance matrix $\bm{\Sigma}$ is computationally expensive and methods for working directly with the sparse precision matrix ($\bm{Q}$; $\bm{Q} = \bm{\Sigma}^{-1}$) are more efficient. We introduce the SPDE approach to working with the precision matrix in Section \ref{sec:software}, but for now, note that we can instead represent our spatial random field as
\[
