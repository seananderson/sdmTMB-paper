...
We will fit counts using a negative binomial \citep[NB2,][]{hilbe2011} distribution, random intercepts for year, spatial and spatiotemporal random fields, and a SVC associated with the NAO.
Centring and scaling variables is especially helpful when estimating SVCs.
NAO is an index so it can be left as is, but including it as a main effect as well helps ensure that the SVCs will have a mean near 0.
...
Mean estimates of the SVC random effect are available in the output of \fct{predict.sdmTMB} in a column starting with \code{zeta\_s} (in this case would appear as \code{zeta\_s\_nao}).
...
Here, we instead specify that we want to return draws for the \code{zeta\_s} ($\zeta_{\bm{s}}$) random field, which is the SVC random field.
