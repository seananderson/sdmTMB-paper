...
paper describes the underlying statistical models, explains how \pkg{sdmTMB} is designed and summarizes its functionality, illustrates its use through three case studies, and
...
where the expected value $\mathbb{E}[.]$ of an observation $y$ at coordinates in space $\bm{s}$ is equal to mean $\mu_s$. That mean is the result of an inverse link function $g^{-1}$ applied to a linear predictor $\eta$. In this case, that linear predictor is the combination of a model matrix $\bm{X}_{\bm{s}}$ multiplied by a vector of coefficients $\bm{\beta}$ and a value from a spatial GRF $\omega_{\bm{s}}$. This spatial GRF represents the effect of latent spatial variables that are not otherwise accounted for in the model. Alternatively, $\omega_{\bm{s}}$ can be thought of representing spatially correlated noise. More simply, $\omega_{\bm{s}}$ represents a ``wiggly'' surface with an expected value of zero that is added to the expectation in link space.

The vector of $\omega_{\bm{s}}$ ($\bm{\omega}$) is assumed drawn from a multivariate normal distribution with covariance matrix $\boldsymbol{\Sigma}_\omega$ constrained by some covariance function that defines the rate that spatial covariance decays with
distance.
Various covariance functions are possible, but a popular and flexible function is the Mat\'ern (REF) (FIGURE TODO).
If we define $\lVert d \rVert$ as the Euclidean distance between spatial points $\bm{s}_j$ and $\bm{s}_k$, we can represent the Mat\'ern covariance $\Phi$ as
\[
\Phi\left( \bm{s}_j, \bm{s}_k \right) = \frac{\tau^2}{2^{\nu - 1}\Gamma(\nu)}
\left(\kappa \lVert d \rVert \right)^\nu K_\nu \left( \kappa \lVert d \rVert \right).
\]
The parameter $\tau^2$ controls the spatial variance, $\Gamma$ represents the Gamma
function, $K_\nu$ represents the modified Bessel function of the second kind,
and $\kappa$ represents the spatial decorrelation rate.
The parameter $\nu$
controls the smoothness of the covariance function, but can be fixed at $\nu = 1$ to take advantage of the SPDE \citep{lindgren2011}.
The parameters $\kappa$ and $\tau$ can be converted to two useful values: (1) correlation range as $\textrm{range} = \sqrt{8} / \kappa$ and (2) marginal standard deviation as $\sigma = 1 / \sqrt{4 \pi \exp \left(2 \log(\tau) + 2 \log(\kappa) \right) }$. The range is the distance that correlation decays to $\approx 0.13$.

The assumption that correlation decays equally in all directions can be relaxed to allow for geometric anisotropy
\[
\Phi\left( s_j, s_k \right) = \frac{\tau^2}{2^{\nu - 1}\Gamma(\nu)}
\left(\kappa \lVert d \rVert \bm{H} \right)^\nu K_\nu \left( \kappa \lVert d \rVert \bm{H} \right).
\]
...
In practice, inversion of the covariance matrix $\bm{\Sigma}$ is computationally expensive; methods of working directly with the sparse precision matrix ($\bm{Q}$; $\bm{Q} = \bm{\Sigma}^{-1}$) are much more efficient. We introduce the SPDE approach to working with the precision matrix in the software section, but for now, note that we can instead represent our spatial random field as
\[
\boldsymbol{\omega} \sim \mathrm{MVNormal}\left(\boldsymbol{0}, \boldsymbol{Q}^{-1}_\omega\right),
\]
...
In this case we assume the spatiotemporal random fields are independent at each time step, but we could alternatively assume they are structured as a random walk or autoregressive process (demonstrated in examples below). The spatiotemporal random fields represent latent variables causing spatial correlation that changes with each time step.

\subsection{Additional model components}

In practice, the above models can become considerably more complex by, for example, letting coefficients vary though time, letting coefficients vary through space \citep{hastie1993} as random fields, or adding random intercepts by grouping factors. All these components are additive in link space.
Adopting the notation ``main'' for main effects, ``tvc'' for time-varying coefficients, and ``svc'' for spatially varying coefficients, a maximally complex model could be written as
\[
\begin{aligned}
\mu_{\bm{s},t} &=
...
\end{aligned}
\]
where the $\bm{X}$ represent model matrices, $\bm{\gamma_t}$ represents a vector of coefficients that are allowed to vary through time as random walks or AR(1) processes, $\zeta_{\bm{s}}$ represents spatially varying coefficients following a GRF, $\alpha_g$ represents IID random intercepts by group $g$ ($\alpha_g \sim \operatorname{Normal} \left(0, \sigma_\alpha^2 \right)$), $O$ represents an offset variable \citep[][p.~206]{mccullagh1989} (e.g., log sampling effort), and $\omega_{\bm{s}}$ and $\epsilon_{\bm{s},t}$ represent spatial and spatiotemporal intercept random fields as before (Figure~\ref{fig:diagram}). We demonstrate these model components in the examples later in this paper.

\subsection{Delta models}

So far we have described models with one linear predictor and one family (common terminology in \proglang{R} packages for the combination of an observation likelihood and link). Frequently, data are better represented with two-part ``delta'' or ``hurdle'' models, which include linear predictors and observation distributions for two processes: zero vs.\ non-zero values and positive values \citep{aitchison1955}. Here, we describe two types of delta models. We drop the space and time subscripts for simplicity and use $\bm{X_1} \bm{\beta_1}$ and $\bm{X_2} \bm{\beta_2}$ to refer to generic model matrices and parameter vectors for the first and second model components.

\subsubsection{Traditional delta models}

Using $p$ to denote the probability of a non-zero observation and $r$ to represent the expected rate for positive data, we can construct linear predictors in link space
\[
\begin{aligned}
...
An additional delta-model is possible that has several advantages over the logit-log delta model \citep{thorson2018poisson}. The primary advantage is that both linear components use a log link so the effects are additive in log space and can be combined. There are still two linear predictors, but they represent a theoretical group number $n$ and a theoretical weight per group $w$ \citep{thorson2018poisson}.
\[
\begin{aligned}
...
\end{aligned}
\]
The first part is the complementary-log-log inverse link \citep[][p.~31]{mccullagh1989} (which has its roots in a Poisson process) but the group number $n$ also enters into the expected positive rate $r$. These probabilities $p$ and positive rates $r$ are then entered into a Bernoulli and positive observation distribution as before
\[
\begin{aligned}
\mathrm{I}(y > 0) &\sim \operatorname{Bernoulli} \left( p \right),\\
y | y > 0 &\sim \operatorname{Positive-distribution} \left( r, \phi \right).
