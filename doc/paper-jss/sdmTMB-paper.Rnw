...
\Rev{A1}We note that caution is warranted in performing model selection via marginal AIC for models involving penalized smoothing (spatial or otherwise) \citep[e.g.][]{greven2010, safken2021}.
...
Methods to estimate effective degrees of freedom were recently demonstrated in \citep{thorsonXX}, but are not yet included in \sdmTMB.

\subsection{Comparing models with cross validation}

As an alternative to AIC, we can conduct model comparison with cross validation via \fct{sdmTMB\_cv}.
...
$$
\mathrm{lpd} = \sum_{i=1}^{n} \ell (\widehat{y_i} | y_i),
$$

where $n$ represents the number of left-out data points, $\ell$ represents the log likelihood, $y_i$ represents left-out data point $i$, and $\widehat{y_i}$ represents the prediction for left-out data point $i$ with the fixed effects at their MLE and random effects at their Empirical Bayes estimate.
...
We include the argument \code{extra\_time}, which represents all time steps to include in the latent process, such that any autoregressive processes represent annual time steps as opposed to the irregular years of data collection.
