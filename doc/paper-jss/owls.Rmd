...
We note that the magnitude of some log-likelihood gradients is slightly large for some fixed effects.

```{r}
fit_owl$gradients
```

We can run additional optimization steps with a Newton optimizer using the\
`run_extra_optimization()` function to further reduce these gradients.

```{r owl-newton, results='hide', cache=TRUE}
...
For the purposes of building this appendix quickly, we will not run this code.
...
```{r shapes, echo=FALSE}
if (!file.exists(here::here("data/ne_10m_lakes.rds"))) {
  ...
  unzip(here::here("data/ne_10m_lakes.zip"), exdir = here::here("data/ne_10m_lakes.rds"))
}
```
...
lakes <- sf::st_read(here::here("data/ne_10m_lakes.rds"), quiet = TRUE)
