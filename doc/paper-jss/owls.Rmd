...
```{r mesh-owls, echo=TRUE, out.width="4in", fig.cap="This mesh uses an Albers projection divided by 100000 to give units of 100 km and a cutoff distance of 1.5 units (or 150 km).", fig.pos='ht', fig.align='center'}
...
We can run additional optimization steps with a Newton optimizer using the `run_extra_optimization()` function to further reduce these gradients.
...
We can confirm that the gradients are smaller.
...
```{r qqnorm-owls, out.width="60%", fig.cap="Randomized quantile residuals.", fig.pos='ht', fig.align='center'}
qqnorm(pred$resid)
qqline(pred$resid)
```
...
To calculate these, we have to use a version of our model fit without REML, which we can quickly obtain with the `update()` method.
...
We can more formally test this with functions from the \pkg{DHARMa} package.
...
```{r plot-all-effects-owls, out.width="95%", fig.asp=0.8, echo=FALSE, eval=TRUE, dpi=300, fig.cap="Predicted winter owl counts that incorporate all fixed effects and random effects.", fig.pos='ht', fig.align='center'}
ggplot(data = p_proj) +
...
```{r plot-spatial-effects-owls, echo=FALSE, eval=TRUE, out.width="5in", dpi=200, fig.cap="Spatial random effects that apply across all years.", fig.pos='ht', fig.align='center'}
ggplot(data = p_proj) +
...
```{r plot-spatiotemporal-effects-owls, out.width="95%", fig.asp=0.8, echo=FALSE, eval=TRUE, dpi=300, fig.cap="Spatiotemporal random effects.", fig.pos='ht', fig.align='center'}
...
```{r plot-fixed-effects-owls, out.width="95%", fig.asp=0.8, echo=FALSE, eval=TRUE, dpi=300, fig.cap="Predictions for each year based on only the non-random field elements.", fig.pos='ht', fig.align='center'}
ggplot(data = filter(p_proj, year > 1978)) +
  geom_point(
