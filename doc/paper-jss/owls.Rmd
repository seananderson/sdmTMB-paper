...
# Example: spatially varying effect on owl counts
...
fit_owl2 <- run_extra_optimization(fit_owl, nlminb_loops = 0, newton_loops = 1)
```

```{r cache-negb2-2, echo=FALSE, eval=!rds_saved2}
saveRDS(fit_owl, here::here("data/snow_w_main_effect_0_150km2.rds"))
```
...
We can also test for zero-inflation using the `simulate.sdmTMB()` function and a couple functions from the \pkg{DHARMa} package.
...
s_nb2 <- simulate(fit_owl2, nsim = 500)
...
```{r theme-owls, cache=FALSE, echo=FALSE}
...
```{r plot-map-ci, echo=FALSE, eval=TRUE, out.width="90%", cache=FALSE, fig.asp=0.3}
...
```{r plot-all-effects-owls, echo=FALSE, eval=TRUE, cache=FALSE, dpi=220}
ggplot(data = p_proj) +
