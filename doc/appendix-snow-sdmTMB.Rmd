...
m <- sdmTMB(count ~ 1 + nao + (1|year_f), spatial_varying = ~ 0 + nao,
...
More accurate residuals can be generated using STAN, but this is a very slow process!
...
In order to produce pretty maps of these predictions, we back transform the spatial coordinates into the same units as our original projection and then convert back into an sf object so that the plot axis will be in latitude and longitude. 
...
```{r shapes, warning=FALSE, echo=TRUE, message=FALSE}
...
```{r plot-map, echo=TRUE, eval=T, cache=FALSE}
ggplot(data = p_proj) + 
...
```{r plot-all-effects, echo=TRUE, eval=T, cache=FALSE}
...
```{r plot-fixed-effects, echo=TRUE, eval=T, cache=FALSE}
ggplot(data = filter(p_proj, year > 1995)) + 
...
```{r plot-spatial-effects, echo=TRUE, eval=T, cache=FALSE}
ggplot(data = p_proj) + 
...
```{r plot-spatiotemporal-effects, echo=TRUE, eval=T, cache=FALSE}
ggplot(data = filter(p_proj, year > 1995)) + 
