...
```{r mesh, echo=TRUE, cache=FALSE, out.width="70%"}
...
It might also be appropriate to include count ID as another random effect (`(1 | CBCID)`) to account for consistent differences in habitat and human observers between count circles. 
...
To do this, we can use the output from the joint precision matrix simulation draws by using the `predict()` function again but this time indicating a number of simulation draws (e.g., `sims = 500`) and what component we want estimated (`sims_var = "zeta_s"`). 
We also extract the same number of drams for (this time `n_sims = 500`) in the `spread_sims()` function for the main effect of NAO. 
The simulation draws for both should then be combined before adding the median and lower and upper quantiles from these draws to our prediction data frame.
...
```{r plot-map, echo=TRUE, eval=TRUE, out.width="100%"}
ggplot(data = p_proj) +
  geom_point(
    data = p_mean,
  ...
  ggtitle("Map of combined main and spatially varying effect of NAO on Snowy Owl count") +
  theme(legend.position = c(0.1, 0.07), axis.title = element_blank())
```

```{r plot-map-ci, echo=F, eval=TRUE, out.width="100%"}
...
```{r plot-all-effects, echo=TRUE, eval=TRUE}
ggplot(data = p_proj) +
  geom_point(
    data = p, aes(X, Y, colour = exp(est)), size = 0.4, alpha = 0.5
  ) +
  coord_sf(
...
```{r plot-spatial-effects, echo=TRUE, eval=TRUE}
ggplot(data = p_proj) +
...
```{r plot-spatiotemporal-effects, echo=TRUE, eval=TRUE}
...
```{r plot-fixed-effects, echo=TRUE, eval=TRUE}
ggplot(data = filter(p_proj, year > 1978)) +
  geom_point(
