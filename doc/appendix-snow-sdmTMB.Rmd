...
dat <- readRDS(here::here("owls/SNOW_data.rds"))
glimpse(dat)
```
...
mesh <- make_mesh(dat, xy_cols = c("X", "Y"), cutoff = 1.5)
```
...
Next, we will plot the counts through time to see if there is an obvious overall trend, or relationship with NAO. 

```{r explore, fig.width = 8}
ggplot(dat, aes(year, log(count+1), colour = nao)) + 
...
This method of checking residuals is not guarenteed to be accurate, so an additional step using STAN could be added.
...
In order to produce pretty maps of these predictions, we back transform the spatial coordinates into the same units as our original projection and reproject. 
One can also get continental coastlines from the package rnaturalearth and outlines of the great lakes from   http://www.naturalearthdata.com/downloads/10m-physical-vectors/ and transform them to the correct projection using `sf::st_transform(crs = proj)`.

```{r proj-p, echo=TRUE, eval=FALSE}
p <- pred %>% mutate(
  X = X * 100000,
  Y = Y * 100000
)
...
```{r plot-map, echo=TRUE, eval=FALSE}
ggplot(data = p_proj) +
  geom_point(data = p_mean, aes(X,Y, colour = nao_effect, size = mean_est_count), alpha = 0.5) +
  geom_sf(data = coast, fill = NA) +
  geom_sf(data = lakes, fill = NA) +
  coord_sf(xlim = c(min(p_proj$X)-50000, max(p_proj$X)-50000), # adjust buffers slightly
           ylim = c(min(p_proj$Y), max(p_proj$Y))) +
  scale_colour_viridis_c(
    guide = guide_colourbar(direction = "horizontal", title.position = "top", label.position = "bottom")) +
...
There are four other kinds of predictions that we get out of the model. 
We can plot our predictions of winter owl counts that incorporate all fixed effects and random effects:

```{r plot-all-effects, echo=TRUE, eval=FALSE}
...
We can also look at just the fixed effects, but in this case these only include the overall effect of mean annual NAO:
...
We can look at the spatial random effects that represent consistent deviations in space that are not accounted for by our fixed effects. 
...
```{r plot-spatial-effects, echo=TRUE, eval=FALSE}
ggplot(data = p_proj) +
...
Finally, we can look at the spatiotemporal random effects that represent deviations from the fixed effect predictions and the spatial random effect deviations. 
