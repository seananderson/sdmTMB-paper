...
```{r check, echo=FALSE, include = FALSE, cache=FALSE, eval=TRUE}
if (rds_saved2) {
...
If our data was zero-inflated or overdispersed, we might have needed to use a delta (aka. hurdle) model instead. 
This is not built in to sdmTMB, but can be achieve using the steps in the vignette [Fitting delta models with sdmTMB](https://https://pbs-assess.github.io/sdmTMB/articles/delta-models.html).
