...
The above example used the default `bias_correct = FALSE` in `get_index()`. The `bias_correct` argument is a feature of TMB for models with random effects, and more detils can be found in the TMB documentation [https://github.com/kaskr/adcomp/wiki](https://github.com/kaskr/adcomp/wiki) or [Thorson & Kristensen (2016)](https://www.sciencedirect.com/science/article/abs/pii/S0165783615301399?via%3Dihub). Leaving `bias_correct = FALSE` is fine for experimentation or a quick example, but will be biased due to the nonlinear (`exp()`) transformation of the random effects. Setting `bias_correct = TRUE` implements the generic by his correction algorithm that should correct for this, but can take considerable time and memory.
...
saveRDS(ind, "sdmTMB-index-sim.rds")
ggplot(ind, aes(year, est / 1000)) +
  geom_line() +
  geom_ribbon(aes(ymin = lwr / 1000, ymax = upr / 1000), alpha = 0.4) +
  ylab("Biomass (t)")
