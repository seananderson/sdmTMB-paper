...
We must then create a mesh object that contains matrices to apply the SPDE approach:

```{r, echo=TRUE}
mesh <- make_mesh(pcod, xy_cols = c("X", "Y"), cutoff = 10)
```

Here, `cutoff` defines the minimum allowed distance between points.
...
And use our data frame and mesh to fit a spatial model:
...
Next we see any estimated fixed effects, the Tweedie dispersion and power parameters, the \mbox{Mat\'ern} range distance, marginal spatial standard deviation, and the negative log likelihood at convergence.
...
```{r}
g_pred <- ggplot(p, aes(X, Y, fill = exp(est))) + geom_raster() +
  scale_fill_viridis_c(trans = "sqrt", option = "C") + coord_fixed() +
...
se <- apply(p3, 1, sd)
p$se <- se
g_pred_se <- ggplot(p, aes(X, Y, fill = se)) + geom_raster() +
  scale_fill_viridis_c(trans = "log10", option = "C") + coord_fixed() +
  labs(fill = "Predicted\ndensity (units)", x = "UTMs E (km)", y = "UTMs N (km)") +
  theme(legend.position = c(0.2, 0.2), legend.key.height = unit(10, 'pt'))
```
...
g_smooth <- ggplot(p_smooth,
  aes(depth, exp(est), ymin = exp(est - 1.96 * est_se),
...
```{r, warning=FALSE, message=FALSE, cache=TRUE, eval=FALSE, echo=TRUE}
fit2 <- sdmTMB(
  density ~ s(depth), 
