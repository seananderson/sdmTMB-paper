...
g_omega <- ggplot(bc_coast_proj) +
  ...
  theme(panel.grid.major = element_line(colour = "grey90"))
cowplot::plot_grid(g_mesh, g_smooth, g_omega, g_pred, labels = "auto", align = "hv")
```
...
The reasons for the interannual variation in the number of individuals migrating south are still not well understood, but seem to be related to high abundances of food during the breeding season and therefore sharp increases in breeding ground population densities [@robillard2016]. 
...
We fit counts observed in North America during annual Christmas Bird Counts (**REF**) using a negative binomial ('NB2') distribution, random intercepts for year, spatial and spatiotemporal random fields, and a spatially varying coefficient associated with NAO (**see appendix for more detail**):
...
We found a weak positive effect of annual mean NAO on overall counts, but also a southeast to northwest gradient in the intensity of this effect.
This result would be consistent with owls closer to the Atlantic coast and those migrating the furthest south being the most effected by NAO (Fig. \@ref(fig:owl-nao)). 
...
```{r owl-nao, fig.cap="(ref:owl-nao-fig)", out.width="3.9in", fig.align='center'}
