...
VAST is also built on TMB and, for univariate data, should be able to fit equivalent models with some minor differences in functionality (e.g., VAST lacks penalized smoothers and the Matérn PC prior).
The difference lies mainly in user interfaces and considerable additional functionality for multivariate response data and delta/hurdle models in VAST.
...
Two major functions included in sdmTMB but not INLA/inlabru are anisotropy and non-linear threshold covariates (TODO: actually true??).
We also think the sdmTMB interface will be more familiar than inlabru to users of `glm()`, lme4, mgcv, or glmmTMB and sdmTMB works with regular R data frames whereas inlabru works with `sp` class data objects. 
