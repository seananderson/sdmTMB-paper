...
The Matérn standard deviations are estimated separately for the various fields and the range---the distance at which spatial correlation decays to $\sim 0.13$ [@lindgren2015]---can be shared (`share_range = TRUE`) or estimated separately (`share_range = FALSE`).
...
The smoothers in sdmTMB can mimic most `mgcv::s()` smoothers including bivarate smoothers (`s(x, y)`), smoothers varying by continuous or categorical variables (`s(x1, by = x2)`), cyclical smoothers `s(x, bs = "cc")`, and smoothers with specified basis dimensions `+ s(x, k = 4)`.
...
The included TMB `.cpp` file also provides a high-quality, bug-free, and tested model template that can be modified to add additional features.
...
We thank M. Lindmark, P.M. Regular, J.T. Thorson, J. Watson, ..., TODO.
