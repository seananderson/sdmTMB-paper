...
abstract: |
    ...
    | 2. Here, we introduce the R package sdmTMB, which implements predictive-process SPDE- (stochastic partial differential equation) based spatial and spatiotemporal models. Estimation is conducted via maximum marginal likelihood with Template Model Builder (TMB) but scales to penalized likelihood or full Bayesian inference. We describe the basic functionality of sdmTMB and the statistical model, illustrate the package's use through two case studies, and compare the functionality, speed, and interface to related software.
