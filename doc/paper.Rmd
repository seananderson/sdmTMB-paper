...
abstract: |
    ...
    | 2. Here, we introduce the R package sdmTMB, which extends the flexible interface familiar to users of glm(), lme4, glmmTMB, or mgcv to include spatial and spatiotemporal latent fields using a predictive-process SPDE-(stochastic partial differential equation) based approach. Estimation is conducted via maximum marginal likelihood with Template Model Builder (TMB), although penalised likelihood and Bayesian inference (via tmbstan/Stan) are also possible. We describe the statistical model, illustrate the package's use to advance research and monitoring through two case studies, and compare the functionality, speed, and interface to related software.
    | 3. The case studies illustrate sdmTMB’s unmatched flexibility and useability by implementing penalised smoothers, non-stationary processes (time-varying and spatially varying relationships), and anisotropy (asymmetry in spatial correlations that commonly occurs along ecological gradients and is not available in INLA/inlabru, the most commonly used packages for this class of models). Modelling of these ecologically ubiquitous, but computationally complex, relationships is further aided by built-in handling of delta/hurdle models, break-point effects, abundance index standardisation, and diagnostic tools including out-of-sample cross validation. Finally, we demonstrate that sdmTMB is 4–16 times faster than INLA or inlabru.
    | 4. sdmTMB's accessible interface has already opened this useful class of models to a wider audience within species distribution modelling. We anticipate its application to the full spectrum of novel ecological problems that involve geostatistical data will prove equally fruitful.
    | Keywords:
...
We ran a simple speed comparison between sdmTMB, inlabru/INLA, and mgcv for fitting an SPDE spatial random field model to 1000 data points with Gaussian error across a range of mesh resolutions (Fig. \@ref(fig:timing), Appendix 6).
...
alternatively, because VAST focuses on multivariate delta models and fisheries applications (Appendix 5), users fitting "simple" univariate spatial/spatiotemporal GLMMs in non-fisheries contexts may find sdmTMB more straightforward to use.
