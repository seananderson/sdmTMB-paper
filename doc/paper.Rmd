...
Our aim is not to replace any of the above-mentioned statistical packages, but to provide a fast, flexible, and user-friendly interface for a specific class of models that should be familiar to users of lme4, glmmTMB, or mgcv [@wood2017a].
...
Spatial and spatiotemporal models constructed with `sdmTMB` may be viewed as spatially expicit models, in the sense that they estimate parameters of a user-specified spatial covariance function, and these estimates may be converted into interpretable quantities common in spatial statistics (spatial decay, range). We contrast these approaches with semi- and non-parametric approaches that do not estimate spatial covariance functions (e.g. `randomForest` [@liaw2002], `MaxEnt` [@phillips2006], some smooths in `mgcv`). By default, sdmTMB implements the Matérn covariance (kernel) (cite Rue et al?) -- this function can accomodate a range of shapes, but also be anisotropic (covariance in a latitudinal direction may be different than in longitudinal direction).
...
GLMMs are available in several widely used packages, including `glmmTMB` (Brooks et al. 2017), `mgcv` [@wood2004], and `lme4` (Bates et al. 2015), and to allow comparison with these packages, `sdmTMB` allows for spatial and spatiotemporal effects to be removed.
...
Following glmmTMB, we allow AIC to be calculated for each sdmTMB model (Akaike XXXX). Because of the well documented biases of AIC with hierarchical or mixed-effects models (can cite Vaida and Blanchard or another) we also provide functionality to perform estimation via k-fold  cross-validation. Our wrapper function `sdmTMB_cv` allows for any number of folds, with assignments being user-specified or chosen randomly. The predictive log-density is calculated for each fold, and then summarized across folds for a total measure of the predictive accuracy of a model.  

<!-- Paragraph on derived quantities here-->
The results from a model fit with sdmTMB can be used to generate quantities of interest, especially relevant to population status. Estimated parameters from the Matérn covariance function can be used to describe the spatial variance and Matérn range (= $\sqrt{8} / \kappa$). We provide additional functionality for generating population-level summaries for each time slice; these metrics include the fraction of area occupied (?), 2-dimensional center of gravity (via our `get_cog()` function), and total population trend (via our `get_index()` function).    
