...
```{r diagram, fig.cap="(ref:diagram-fig)", out.width="1.03\\textwidth"}
...
A benefit of the geostatistical approach over CAR or similar models is that the parameters describing spatial covariance can be estimated (e.g., spatial variance and range---the distance at which points are effectively independent).
...
Derived parameters from the Matérn covariance function can be used to describe the marginal spatial variance and Matérn range. <!-- This sentence felt tacked on at the end. Maybe it fits better here? -->
...
While the sdmTMB model is fit by maximum marginal likelihood, Bayesian options are also supported.  
...
However, after rapid model exploration with maximum likelihood, one can optionally pass an sdmTMB model to the R package tmbstan [@monnahan2018] to estimate the joint posterior distribution.
This option can be fully Bayesian, if penalized likelihoods are included by assigning priors to model parameters (`?sdmTMBpriors`).
These priors can also be useful in cases where maximum likelihood estimation is difficult because of identifiability issues or relatively flat likelihood surfaces, or to impart prior information or achieve regularization.
...
<!-- Would this be an appropriate place to say something about the sim options, possibly specifically for getting uncertainty on spatial predictions? I don't know how easy that is from the other packages... -->
