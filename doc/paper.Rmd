...
```{r rnaturalearthhires, include=FALSE, echo=FALSE}
...
The reasons for the interannual variation in the number of individuals migrating south are still not well understood, but seem to be related to very high abundances of food during the breeding season and therefore sharp increases in breeding ground population densities [@robillard2016]. 
...
Because both productivity and the choice of wintering location could be influenced by climate, we tested for a spatially variable effect of annual mean NAO index on winter abundance across the southern boundary of their winter distribution. 
We fit counts observed in North America during annual Christmas Bird Counts (**REF**) using a negative binomial distribution, random intercepts for year, both spatial and spatiotemporal random fields, and a spatially varying effect of NAO (**see appendix for more detail**): 

```{r owl-fit, warning=FALSE, message=FALSE, cache=TRUE, echo=TRUE, eval=FALSE}
fit_owls <- sdmTMB(
  count ~ 1 + nao + (1|year_f),
  spatial_varying = ~ 0 + nao,     
  family = nbinom2(link = "log"), data = snow, mesh = mesh, reml = TRUE,  
  time = "year", spatial = "on", spatiotemporal = "IID"
  )
```

We found a weak positive effect of annual mean NAO on overall counts, but that there was also a southeast to northwest gradient in the intensity of this effect that would be consistent with owls closer to the Atlantic coast and migrating the furthest south being the most effected by NAO (Fig. \@ref(fig:owl-nao)). 

(ref:owl-nao-fig) A spatially varying effect of mean annual NAO on counts of Snowy Owls observed on annual Christmas Bird Counts between 1979 and 2020. 
Circles represent all count locations and are roughly scaled to the mean number of owls observed each year (range: 0 to 8).

```{r owl-nao, fig.cap="(ref:owl-nao-fig)", out.width="3.5in", fig.align='center'}
owl <- here::here("figs", "owl_nao_effect.pdf")
if (!file.exists(owl)) {
  download.file("https://www.dropbox.com/s/3aco85mizuplz7z/owl_nao_effect.pdf?dl=1", 
    destfile = owl)
}
