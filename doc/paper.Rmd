...
Packages such as VAST [@thorson2019] and inlabru [@bachl2019] are powerful user interfaces to fit spatial models that use the SPDE, but they either lack a modular interface familiar to those who have used lme4 or glmmTMB or some of the flexibility for the particular class of models discussed here.
...
Geostatistical data simply refers to data observed at specific spatial coordinates. The data can be collected across discrete points in time. Areal data (data on polygons or grids) may be analyzed using other spatial models, including conditional autoregressive models (CAR). sdmTMB can also fit models with areal data if each polygon has an associated centroid (observations are treated as replicates). A benefit of the sdmTMB approach over CAR or similar models is the parameters describing spatial covariance can be estimated (e.g. range).
...
We can make predictions with the `predict()` method and optionally use the `newdata` argument to predict on a new data frame with any locations and values for the predictor columns.
<!-- Here, we will predict on a 2x2 km grid that covers the entire region of interest so we can visualize the predictions spatially (or calculate a standardized population index with a spatiotemporal model). -->
...
<!-- could add: 'and functionality' to the above if next sentence deemed redundant. -->
<!-- The major difference lies in VAST's considerable additional functionality for multivariate response data (REF) and delta/hurdle models (REF). -->
...
<!-- I think either this final sentence or the 3rd sentence are a bit redundant. I suggest removing the 3rd one here. -->
