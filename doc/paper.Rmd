...
<!-- While measured variables can be accounted for via predictors in a model (e.g., including latitude, recognized as an important ecological gradient as far back as von Humboldt (1808)), unmeasured variables (e.g., everything influencing species abundance but not explicitly modelled) can still cause residual spatial correlation. -->
...
While simple on the surface, such models quickly become computationally limiting due to the need to invert large matrices in order to keep track of covariation among data [e.g., @latimer2009].
...
<!-- or areal/grid data with centroids instead of CAR/SAR models -->
<!-- add CAR/SAR to table -->
...
<!-- (observations are treated as replicates). -->
A benefit of the geostatistical approach over CAR or similar models is the parameters describing spatial covariance can be estimated (e.g., range---the distance at which points are effectively independent).
...
<!-- highlight where each one excels ... -->
<!-- no CV built into VAST or priors -->
<!-- discussion integrates across speed, familiarity and table 1 -->
<!-- results paragraph on Table 1 -->
<!-- familiarity is an important word here -->
<!-- add section on functionality comparison -->
