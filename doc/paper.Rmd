...
Though `sdmTMB` has been developed with spatial modeling as a primary focus, all of the statistical models fall under the class of generalized linear mixed models (GLMMs). Non-spatial GLMMs are available in several widely used packages, including `glmmTMB` (Brooks et al. 2017) and `lme4` (Bates et al. 2015), and to allow comparison we also allow non-spatial models to be constructed with `sdmTMB`. The general form of a GLMM is  $g(\bf{\mu}) = \bf{Xb} + \bf{Zu}$, where the inverse link function $g(x)$ relating the mean $\bf{\mu}$ to fixed effect predictors in the design matrix $\mathbf{X}$ with estimated coefficients $\bf{b}$ and random effects with the design matrix $\bf{Z}$ with estimated coefficients $\bf{u}$. The matrix $\bf{Z}$ allows random effects to be structured (e.g. intercepts varying by group) or unstructured, with $\bf{u}$ representing random variation.

For clarity, we partition the random effects $\bf{Zu}$ into a number of more interpretable components, each of which can optionally be included in the model. Using our notation, the full `sdmTMB` model can be written as $$g(\mu_{s,t}) = X_{s,t}\bf{b} + \omega_{s} + \epsilon_{s,t} + \bf{\alpha_{g}} + X_{s,t}\gamma_{t} + X_{s,t}\zeta_{s}$$. In this form, $\omega_{s}$ represents a random spatial component (spatial field with mean 0), $\epsilon_{s,t}$ represents a spatiotemporal component (spatial field with mean 0), $\alpha_{g}$ represents random intercepts by group $\alpha_{g}\sim N(0,\sigma_\alpha)$, $\gamma_{t}$ represents time-varying coefficients modeled as a random walk, $\gamma_{t} \sim N(\gamma_{t-1},\sigma_\gamma)$, and $\zeta_{s}$ represents spatially varying coefficients (modeled as a spatial field). All of the spatial fields in this model are constructed using Gaussian random fields, where the random effects are structured as multivariate normal: $\omega_s \sim MVN(\bf{0},\bf{\Sigma}_\omega)$, $\epsilon_{s,t} \sim MVN(\bf{0},\bf{\Sigma}_{\epsilon})$, and $\zeta_s \sim MVN(\bf{0},\bf{\Sigma}_\zeta)$. By default, we assume that if spatiotemporal fields are included, they are independent and identically distributed; however, additional options in `sdmTMB` allow them to be modeled as a random walk or AR(1) process. Additional flexibility is included in relaxing assumptions about linear fixed effects $\bf{Xb}$; sdmTMB allows any covariates to be modeled as smooth functions using the same generalized additive model syntax in `mgcv` (Wood 2003), or modeled as threshold (e.g. hockey stick models; Barrowman et al. 2003).  
