...
editor_options:
  markdown:
    wrap: sentence
abstract: |
    | \noindent 1. Geostatistical data are spatially referenced observations of some spatial process---they might also be collected across time. Such data are common in ecology and can reveal ecological processes and improve conservation and management through applications such as species distribution models (SDMs). However, appropriate models to analyze geostatistical data, such as generalized linear mixed effects models (GLMMs) with Gaussian random fields, are often computationally intensive and challenging to implement, interpret, and evaluate.
    | 2. Here, we introduce the R package sdmTMB, which implements predictive-process SPDE- (stochastic partial differential equation) based spatial and spatiotemporal models. Estimation is conducted via maximum marginal likelihood with Template Model Builder (TMB). We describe the basic functionality of sdmTMB and the statistical model, illustrate the package's use through two case studies, and compare the functionality, speed, and interface to related software.
    | 3. We highlight advantages of using sdmTMB for this class of models: (1) sdmTMB provides a flexible interface familiar to users of `glm()`, lme4, glmmTMB, or mgcv; (2) estimation is often faster than alternatives; (3) sdmTMB provides simple out-of-sample model validation; (4) non-stationary processes are easily constructed with a formula interface (time-varying coefficients, spatially varying coefficients); and (5) sdmTMB includes features not available as a combination in related packages (e.g., penalized smoothers and break-point effects, anisotropy, population abundance index standardization).
