...
editor_options:
  markdown:
    wrap: sentence
abstract: |
    | \noindent 1. Geostatistical data---spatially referenced observations with some underlying spatial process---are commonly collected in ecology and have much potential for revealing new ecological processes and improving conservation and management. Geostatistical data are used in a diversity of applications, including species distribution models (SDMs); however, appropriate models for analyzing these data, such as generalized linear mixed effects models (GLMMs) with Gaussian random fields, are often computationally intensive and challenging to implement, interpret, and evaluate.
...
Accounting for this residual correlation is important because doing so allows for valid statistical inference [@legendre1989a; @dormann2007], can improve predictions compared to ignoring it [e.g., @shelton2014], and can be of ecological interest itself [e.g., @thorson2019d; @barnett2021].
...
Packages such as VAST [@thorson2019] and inlabru [@bachl2019] are powerful user interfaces to fit spatial models that use the SPDE, but they either lack a modular interface familiar to those who have used lme4 or glmmTMB or lack some of the flexibility for the particular class of models discussed here.
...
f <- here::here("figs", "diagram-2021-12-10.pdf")
if (!file.exists(f)) {
  download.file("https://www.dropbox.com/s/lvoem3ahwkftzjy/diagram-2021-12-10.pdf?dl=1", 
    destfile = f)
}
...
It usually makes sense to convert the spatial coordinates to an equidistant projection such as UTMs such that distance remains constant throughout the study region [e.g., using `sf::st_transform()`, @pebesma2018].
...
Here, `cutoff` defines the minimum allowed distance between mesh vertices in the units of `X` and `Y` (km).
...
We could generate a population index that is independent of sampling locations by predicting from that fit on a grid covering the area of interest and summing the predicted biomass with the `get_index()` function (Fig. \@ref(fig:pcod-fig)E, Appendix 2).
...
owl <- here::here("figs", "owl-nao-effect-2021-12-22.pdf")
if (!file.exists(owl)) {
  download.file("https://www.dropbox.com/sh/p5ydljdycaw9x6n/AAAlaKQArhqP9IHvUSxEq_qWa/owl-nao-effect-2021-12-22.pdf?dl=0", 
    destfile = owl)
}
...
```
<!-- LB: VAST does allow use of penalized splines, but this requires the splines package and effects package for visualization, e.g.: https://github.com/James-Thorson-NOAA/VAST/wiki/Specify-covariates-and-visualize-responses -->

# Package comparisons

There are many R packages capable of fitting geostatistical spatial or spatiotemporal models [e.g., @heaton2017].
sdmTMB, VAST, INLA/inlabru, and spaMM [@rousset2014] are the most closely related, as they all provide a user interface to SPDE-based GMRF models; we also include spBayes as a prominent package that can fit related predictive-process models without the SPDE (Table 1).
...
Results with up to an order of magnitude more data slightly shift the lines up without affecting the overall pattern substantially (not shown).
