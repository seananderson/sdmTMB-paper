...
```{r}
g_pred <- ggplot(bc_coast_proj) + geom_sf(
                                           # colour=NA, fill="white"
                                           fill=NA, colour="gray30"
                                           # fill="gray70", colour=NA
                                          ) +
  geom_raster(data = p, aes(x = X * 1000, y = Y * 1000, fill = exp(est))) +
  coord_sf(datum = st_crs(utm_zone9)) +
  scale_x_continuous(limits = c(230957.7 + 105000, 1157991 - 570000),
    labels = function(x) paste0(x/1000)
  ) +
  scale_y_continuous(limits = c(5366427 + 270000, 6353456 - 513000),
    labels = function(x) paste0(x/1000)
  ) +  
...
g_omega <- ggplot(bc_coast_proj) + geom_sf(
                                           # colour=NA, fill="white"
                                           fill=NA, colour="gray30"
                                           # fill="gray70", colour=NA
                                           ) +
