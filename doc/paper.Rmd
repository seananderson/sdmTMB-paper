...
editor_options:
  markdown:
    wrap: sentence
abstract: |
    | \noindent 1. Geostatistical data---spatially referenced observations related to some continuous spatial phenomenon---are ubiquitous in ecology and can reveal ecological processes and inform management decisions through applications such as species distribution models (SDMs). However, appropriate models to analyze these data, such as generalized linear mixed effects models (GLMMs) with Gaussian random fields, are often computationally intensive and challenging to implement, interpret, and evaluate.
...
knitr::kable(d, format = "latex", caption = "Comparison of functionality between seven R packages that can fit geostatistical GLMMs.",
  ...
  kableExtra::footnote(
    number = c(
      ...
      "Hurdle models possible by fitting components separately",
      "Hurdle models possible by fitting components separately",
      "Technically possible but non-trivial"
    ),
    escape = FALSE
