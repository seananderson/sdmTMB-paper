...
editor_options:
  markdown:
    wrap: sentence
abstract: |
    | \noindent 1. Geostatistical data---spatially referenced observations with some underlying spatial process---are commonly collected in ecology and have much potential for revealing ecological processes and improving conservation and management, expanding on the time series analyses often used for such efforts by quantifying spatial and spatiotemporal relationships. Geostatistical data are used in a diversity of applications, including species distribution models (SDMs); however, appropriate models for analyzing these data, such as generalized linear mixed effects models (GLMMs) with Gaussian random fields, are often computationally intensive and challenging to implement, interpret, and evaluate.
    | 2: Here, we introduce the R package sdmTMB, which implements predictive-process SPDE-based (stochastic partial differential equation) spatial and spatiotemporal models. Estimation is conducted via maximum marginal likelihood with Template Model Builder (TMB). We describe the basic functionality of sdmTMB and the statistical model employed, illustrate its use through two case studies, and compare the functionality, speed, and interface to related software.
    | 3:  We highlight advantages of using sdmTMB for this class of models: (1) sdmTMB provides a flexible interface familiar to users of `glm()`, lme4, glmmTMB, or mgcv; (2) estimation is often faster than alternatives; (3) provides simple out-of-sample model validation; (4) non-stationary processes are easily constructed with a formula interface (time-varying coefficients, spatially varying coefficients); and (5) sdmTMB includes features not available as a combination in related packages (e.g., penalized smoothers and break-point effects, anisotropy, population abundance index standardization).
    | 4: We hope that user-friendly interfaces such as sdmTMB open this useful class of models to a wider audience of users.
    | Keywords: Gaussian Markov random fields, generalized linear mixed effects models, INLA, species distribution modelling, R package, Template Model Builder
---
...
<!-- These data sometimes include a discrete time dimension, such as yearly samples.  -->
<!-- Spatially and spatiotemporally explicit data are increasingly collected in ecology and have the power to reveal new ecological processes and improve ecological management. -->
...
Areal data (data on polygons or grids) may be analyzed using other spatial models, including conditional autoregressive models (CAR) [e.g., @verhoef2018].
...
After rapid model exploration with maximum likelihood, one can optionally pass a sdmTMB model to the R package tmbstan [@monnahan2018] to estimate the joint posterior distribution.
...
<!-- Since this is not particular to your package, maybe the e.g. for how to do this should be moved to the appendix? I do explain the transformation for the owl data, but it could be added to the pcod appendix too? -->
...
<!--
EW: TODO I think this could use a bit more hand holding — things to highlight are (1) predictions can be made for a new dataframe with any locations, 
(2) more detail about why you’re predicting on a grid. 
(3) other uses for predict — e.g. might be helpful to understand marginal effects of covariates for complicated models
-->
...
We could extend this spatial model to be a spatiotemporal model simply by supplying the year column name to the `time` argument and specifying how we want the spatiotemporal random fields to be structured in time.
...
<!-- not familiar with this "irruptive" terminology, please define -->
...
This indicates that the owls most effected by NAO were those closer to the Atlantic coast and those migrating the furthest south. 
...
<!-- We allow AIC to be calculated for each sdmTMB model [`AIC()`, @akaike1974]; however, because of well-documented biases of AIC with mixed-effects models [@liang2008] we provide functionality to perform k-fold cross-validation. -->
...
<!-- Validation of state-space models is challenging, particularly when using the Laplace approximation [@thygesen2017a]. We provide four approaches to assist with model validation: -->
...
(3) A sdmTMB model can be passed to the tmbstan package [@monnahan2018] to sample from the joint posterior with Stan [@carpenter2017], evaluate the accuracy of the Laplace approximation, or evaluate residuals (see `?extract_mcmc`). 
...
(5) The `simulate.sdmTMB()` method or `sdmTMB_simulate()` function can simulate from fitted models or new data to which models can be fit to ensure identifiability, evaluate bias and precision in parameter estimation, or evaluate the consequences of model misspecification.
...
<!-- We note this will be considerably slower to fit than with maximum likelihood and in some cases may require tight priors or fixed parameters to ensure adequate sampling [as in @monnahan2021]. -->
...
```
<!-- LB: VAST does allow use of basis splines, but this requires the splines package and effects package for visualization, e.g.: https://github.com/James-Thorson-NOAA/VAST/wiki/Specify-covariates-and-visualize-responses -->
...
To our knowledge, VAST is the only package to implement spatial [@thorson2015] and spatial dynamic factor analysis [@thorson2016]. 
...
<!-- and spatial empirical orthogonal function (EOF) regression [@thorson2020]. To my reading EOF was not implemented in VAST. It seemed to me its own thing?-->
...
<!-- TODO: add something about the results of the INLA comparison (Appendix 4) and the VAST comparison (Appendix 5) -->
We provide comparisons of the syntax and the reproducibility of results from models fit using INLA (Appendix 4) and VAST (Appendix 5).
<!-- We ran into problems with AR1 spatiotemporal fields in inlabru, otherwise we would have included both INLA and inlabru syntax.
<!-- TODO: mention inlabru here -->
...
We ran a simple speed comparison between sdmTMB, inlabru, and mgcv for fitting a SPDE spatial random field model to 1000 data points with Gaussian error across a range of mesh resolutions (Fig. \@ref(fig:timing), Appendix 6).
...
Results with up to an order of magnitude more data slightly shift the lines up without affecting the qualitative pattern (not shown).
...
<!-- EW: TODO Not sure if Heaton included CAR models, but we might want to talk about them briefly because there’s a lot of packages that include that functionality - including NIMBLE, etc.  -->

How does one choose among the related packages mentioned in this paper to fit SPDE-based geostatistical GLMMs?
Assuming a given package can fit the model of interest (Table 1), we suggest the primary difference is the user interface.
...
Users already familiar with mgcv can fit adapt mgcv to fit similar models with custom code [@miller2019] and INLA/inlabru and mgcv are also general purpose modelling packages.
...
<!-- Future directions para? -->
<!-- multivariate, but only if can keep interface simple -->
<!-- optimized Bayes sampling -->
...
<!-- areal models, neighbours or not, must be CAR or SAR, ... but if have centroid, could use sdmTMB... why not CAR/SAR, spatial process... not 1:1 relationship between distance and spatial correlation? neibour based only... -->
<!-- https://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.701.1636&rep=rep1&type=pdf -->
...
<!-- CAR/SAR models? and why not if have...-->
<!-- EOF paper from Jim-->
<!-- testing that has been done... -->
<!-- https://onlinelibrary.wiley.com/doi/abs/10.1111/gcb.15149 -->
<!-- https://onlinelibrary.wiley.com/doi/abs/10.1111/gcb.15149 -->
