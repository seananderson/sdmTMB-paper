...
<!-- # For submission to MEE as an Application paper -->
...
*[quick draft outline of intro; may be too long]*

Ecological data are often collected in space or in space repeatedly over time. For example... ? While such data are a rich source of information about ecological process, it is challenging to properly modelling them. Data points closer in space and time are usually more correlated to each other than those farther apart, both due to measured and unmeasured variables. While measured variables can be accounted for via predictors in a model, unmeasured ("latent") variables cause residual spatial/spatiotemporal correlation that (1) needs be accounted for valid statistical inference, (2) can improve predictions compared to ignoring it, and (3) can be of ecological interest in itself.

Geostatistical GLMMs (Generalized Linear Mixed Effects Models) with spatially correlated random effects is a class of models appropriate for these data. Similarly to how random intercepts or slopes can account for correlation among groups (e.g., plants counted in plots of land), spatial/spatiotemporal random effects can account for unmeasured variables causing observations to be correlated in space or space and time. A common approach to modelling the random effects is via Gaussian Random fields (GRFs), where the random effects are assumed drawn from a multivariate normal (MVN) distribution, constrained by some covariance function such as the exponential or Matérn. 

While simple on the surface, such models quickly become computationally limiting due to the need to invert large matrices to keep track of covariation among data points. A number of solutions have been proposed, such as predictive processes (e.g., REF), NNGP (e.g., REF), and SPDE + GMRF (e.g., REF). These approaches reduce the scale of the covariance estimation problem while providing a means to evaluate the data likelihood, thereby allowing fitting via Bayesian or maximum likelihood methods. This can make an enormous difference to computational efficiency (REFs). The SPDE approach is a solution popularized via INLA and an implementation in TMB (Template Model Builder) that partially relies on INLA. The SPDE… yada yada GMRF yada yada.

INLA and TMB are template systems for specifying statistical models that can include the SPDE, but they are challenging to use for many applied ecologists and it can be slow to iterate on and experiment with many models when writing bespoke code. 
Packages such as lme4 and glmmTMB let users quickly iterate and explore statistical models, evaluating fit and comparing models, but do not have built-in SPDE functionality. Packages such as VAST and inlabru are powerful user interfaces to spatial models that use the SPDE, but either lack a modular interface familiar to those who have used lme4 or glmmTMB, or lack some of the flexibility for the particular class of models discussed here. We provide a more detailed comparison of related software packages in Table 1 and the Discussion.
...
Here, we introduce our R package sdmTMB, which implements spatial and spatiotemporal GLMMs using TMB, INLA, and Gaussian Markov random fields. One common application is for species distribution models (SDMs), hence the package name. This paper describes the basic functionality of the R package and the underlying statistical model, illustrates its use through X case studies, and concludes with a discussion of additional functionality and related software.
...
# Description
- maybe some overview of spatial models, either here in intro. Contrast with GAMs, and highlight explicit estimation of flexible spatial covariance functions

# Model

Though `sdmTMB` has been developed with spatial or spatiotemporal modelling as a primary focus, all of the statistical models it fits fall under the class of generalized linear mixed models (GLMMs).
GLMMs are available in several widely used packages, including `glmmTMB` (Brooks et al. 2017) and `lme4` (Bates et al. 2015), and to allow comparison we also allow non-spatial models to be constructed with `sdmTMB`.
...
The full `sdmTMB` model is
$$
g(\mu_{\bm{s},t}) = 
\bm{X}_{1,\bm{s},t} \bm{\beta} + 
\bm{\alpha}_g +
\bm{X}_{2,\bm{s},t} \gamma_t + 
\bm{X}_{3,\bm{s},t} \zeta_{\bm{s}} +
\omega_{\bm{s}} + 
\epsilon_{\bm{s},t},
$$
where
...
The random fields are constructed as Gaussian Markov random fields (GMRF), where the random effects are structured as MVN constrained by a \mbox{Mat\'ern} covariance function. 
...
By default, we assume that if spatiotemporal fields are included, they are independent and identically distributed; however, additional options in `sdmTMB` allow them to be modelled as a random walk or first-order autoregressive (AR1) process.
We include additional flexibility in specifying the linear fixed effect matrix $\bm{X \beta}$: `sdmTMB` allows covariates to be modelled as penalized smooth functions (generalized additive models; GAMs) using the same `s()` syntax as in `mgcv` (Wood 2003), or to be modelled with threshold shapes (e.g., hockey stick models; Barrowman et al. 2003).  
