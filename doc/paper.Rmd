...
Point data simply means the data have explicit spatial coordinates (vs. areal data that might represent polygons or grids).
...
Using the notation of a bold capital letter to represent a data vector, a bold lowercase greek letter to represent a parameter vector, the index $\bm{s}$ to represent a point in space, and the index $t$ to represent a point in time,
...
sdmTMB achieves 3 of the 4 design principles outlined in Thorson (REF) for index standardization: it allows area weighting, can distinguish catchability and habitat covariates (by deciding what is fixed when predicting), and conditions on missing covariates (random fields).
...
For some models, TMB/sdmTMB can be considerably faster than INLA/inlabru [@osgood-zimmerman2021], but in our experience, those differences depend on many aspects of the data, SPDE mesh, model, and fitting algorithm settings (TODO Supp fig?).
...
mgcv can fit many similar models to sdmTMB either with various flavours of splines or even with the SPDE approximation to random fields (REF); however, there are some notable advantages of sdmTMB for the specific class of models discussed here.
First, the smoothers typically used with mgcv (thin plate splines, ...), while capable of making equivalent predictions (REF) do not offer the ecological interpretability of a spatially explicit random field model (spatial range and variance).
