...
editor_options:
  markdown:
    wrap: sentence
abstract: |
    | \noindent 1. Geostatistical data---spatially referenced observations with some underlying spatial process---are increasingly collected in ecology and are used in applications such as species distribution models (SDMs). However, appropriate models for these data, such as  generalized linear mixed effects models (GLMMs) with Gaussian random fields, are often computationally intensive and challenging to implement, interpret, and evaluate.
    | 2: Here, we introduce the R package sdmTMB, which implements predictive-process SPDE-based (stochastic partial differential equation) spatial and spatiotemporal models. Estimation is conducted via maximum marginal likelihood with Template Model Builder (TMB). We describe the basic functionality of sdmTMB and the statistical model, illustrate its use through two case studies, and compare the functionality, speed, and interface to related software.
    | 3:  We highlight advantages of using sdmTMB for this class of models: (1) sdmTMB provides a flexible interface familiar to users of `glm()`, lme4, glmmTMB, or mgcv; (2) estimation is often faster than alternatives; (3) sdmTMB allows for simple out-of-sample model validation; (4) non-stationary processes are easily constructed with a formula interface (time-varying coefficients, spatially varying coefficients [SVC]); and (5) sdmTMB includes features not available as a combination in related packages (e.g., penalized smoothers and break-point effects, anisotropy, population index standardization).
