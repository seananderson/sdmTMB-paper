...
Here, we illustrate a simple spatial model fit to Pacific Cod (TODO) trawl survey data from Queen Charlotte Sound.
Our model contains a main effect of depth as a penalized smoother, a spatial random field, and Tweedie observation error.
Our data frame `d` has a column `year` for the year of the survey, `density` for density of Pacific Cod in a given survey tow, `depth` for depth in m of that tow, and spatial coordinates X and Y, which are UTM coordinates in km:

```{r, include=FALSE}
dat <- dplyr::select(pcod, year, density, depth, X, Y)
```
...
Alternatively, we could have created any mesh via the INLA package and supplied it to `make_mesh()`. We can then inspect our mesh object with the associated `plot()` method (see Fig. \@ref(fig:pcod-fig)a):
...
```{r, echo=TRUE, eval=FALSE}
p_new <- predict(fit, newdata = qcs_grid)
```

We show a basic plot of spatial random field, predictions across a depth gradient, and predictions in space in Fig. \@ref(fig:pcod-fig)b--d.

```{r, echo=FALSE, eval=TRUE}
p_new <- predict(fit, newdata = subset(qcs_grid, year == 2003))
```

```{r}
g_pred <- ggplot(p_new, aes(X, Y, fill = exp(est))) + geom_raster() +
  scale_fill_viridis_c(trans = "sqrt") + coord_fixed() +
...
```{r}
nd <- data.frame(depth =
  seq(min(dat$depth), max(dat$depth), length.out = 300))
...
g_smooth <- ggplot(p_smooth,
  ...
  geom_ribbon(fill = "grey65") +
  geom_line(colour = "grey10") +
  labs(x = "Depth (m)", y = "Density (units)") +
  xlim(min(dat$depth), 300) +
  coord_fixed(expand = FALSE, ratio = 1.2)
```

```{r pcod-fig, fig.cap="example output", fig.width=9, out.width="5.2in", fig.asp=1, fig.align='center'}
g_omega <- ggplot(p_new, aes(X, Y, fill = omega_s)) +
  geom_raster() +
...
fit_spatiotemporal <- sdmTMB(
  density ~ s(depth), family = tweedie(link = "log"), data = dat, mesh = mesh
