...
<!-- I'm finding the above a bit awkward. What about: Ideally, this residual correlation should be accounted for because doing so... OR Accounting for this residual correlation is important because doing so... -->
...
<!-- the e.g. here isn't intuitive for me... usually you only get one count from one plot so what is being grouped? Maybe heights of trees on different plots of land? -->
...
For example, sdmTMB provides functionality for generating population-level summaries for each time slice including the center of gravity (`get_cog()`) and total population index (`get_index()`; Fig. \@ref(fig:pcod-fig)E).
