...
Comparing the fixed effects between `sdmTMB()` and `inla()` shows the estimates are very similar:

```{r coef-plot, out.width="5.5in"}
...
Alternatively, we could use `inla.posterior.sample(result = m_inla, n = 1000)` to sample from the INLA model posterior.
...
Or, we can make the prediction with inlabru. To do that, we must recreate `i_year`, create a `SpatialPixelsDataFrame` class data frame, and specify in `formula` to include both the random field (`f`), the factors (`fac`), and use the inverse-link transformation `plogis()`:

```{r inlabru-pred, out.width="6in"}
nd_bru <- nd
nd_bru <- dplyr::left_join(nd_bru, yr_lu)
