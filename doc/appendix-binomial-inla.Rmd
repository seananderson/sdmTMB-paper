...
m <- sdmTMB(present ~ 0 + as.factor(year), 
  ...
  priors = sdmTMBpriors(
    matern_st = pc_matern(
      range_gt = range_min, range_prob = prior_prob, #P(range > 5) = 1-0.05
      sigma_lt = sigma_max, sigma_prob = prior_prob #P(sigma < 5) = 1-0.05
    ),
    b = normal(0, 100)
  ))
...
x <- list()
x[[1]] <- inla.smarginal(m_inla$marginals.fixed$year_factor2011) %>% 
  as_tibble() %>% mutate(variable = "as.factor.year.2011") 
x[[2]] <- inla.smarginal(m_inla$marginals.fixed$year_factor2013) %>% 
  as_tibble() %>% mutate(variable = "as.factor.year.2013") 
x[[3]] <- inla.smarginal(m_inla$marginals.fixed$year_factor2015) %>% 
  as_tibble() %>% mutate(variable = "as.factor.year.2015") 
x[[4]] <- inla.smarginal(m_inla$marginals.fixed$year_factor2017) %>% 
  as_tibble() %>% mutate(variable = "as.factor.year.2017") 
x[[5]] <- inla.smarginal(m_inla$marginals.hyperpar$`Range for i`) %>% 
  as_tibble() %>% mutate(variable = "range") 
x[[6]] <- inla.smarginal(m_inla$marginals.hyperpar$`Stdev for i`) %>% 
  as_tibble() %>% mutate(variable = "sigma_E") 
x[[7]] <- inla.smarginal(m_inla$marginals.hyperpar$`GroupRho for i`) %>% 
  as_tibble() %>% mutate(variable = "ar1_rho") 
...
We can predict on a the grid (`qcs_grid`) provided with the `pcod` data in the sdmTMB package to map occurrence probabilities in each year. 
...
```{r maps, cache=FALSE, out.width="4in"}
ggplot(pred) + geom_raster(aes(X, Y, fill = plogis(est))) + 
  facet_wrap(~year) +   scale_fill_viridis_c() + coord_fixed() 
```

Similar predictions are more complicated to extract from INLA, but can be plotted for the original sample locations.

```{r inla-pred, out.width="4in"}
idx <- inla.stack.index(sdat, 'stdata')$data
d$inla_pred <- m_inla$summary.fitted.values[idx, "mean"]
ggplot(d) + geom_point(aes(X, Y, colour = inla_pred), size = 2) + 
  facet_wrap(~year) + scale_colour_viridis_c() + coord_fixed() 
```
