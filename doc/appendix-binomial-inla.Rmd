...
Note that the standard errors for variance terms are not in natural and not log space and so are a very rough approximation and therefore not reported by the `tidy()` function.
...
The resulting values are in link space and require the appropriate inverse-link transformation, in this case `plogis()`, to produce occurrence probabilities.
...
We can make similar predictions with inlabru using only a few additional steps: we must recreate `i_year`, create a `SpatialPixelsDataFrame` class data frame, and specify in `formula` to include both the random field (`f`), the factors (`fac`), and the appropriate inverse-link transformation:
