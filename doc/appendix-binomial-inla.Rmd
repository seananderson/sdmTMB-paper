...
In this example, we fit a presence-absence species distribution model with fixed-effect intercepts for year and spatiotemporal random fields structured as AR(1). 
...
In INLA a PC-prior can be used for the temporal autoregressive parameter, i.e., a prior that defines the probability the autocorrelation parameter is greater than a given value (e.g., `P(cor > 0) = 0.9`), but for now, sdmTMB only takes a Gaussian prior (e.g., `ar1_rho = normal(0, 1)`).
...
# INLA 
<-- EW: missing a citation in REF below -->
...
We use `control.inla = list(int.strategy = "eb", strategy = "gaussian")` for comparability with TMB (REF).
We use weak priors on the fixed effects (`mean = 0, prec = 1/(100*100)`), which amounts to a `Normal(0, 100^2)` prior.
