...
```{r inla-densities}
x <- list()
x[[1]] <- inla.smarginal(m_inla$marginals.fixed$year_factoryear_factor2007) %>%
  as_tibble() %>%
  mutate(variable = "as.factor.year.2007")
x[[2]] <- inla.smarginal(m_inla$marginals.fixed$year_factoryear_factor2009) %>%
  as_tibble() %>%
  mutate(variable = "as.factor.year.2009")
x[[3]] <- inla.smarginal(m_inla$marginals.fixed$year_factoryear_factor2011) %>%
  as_tibble() %>%
  mutate(variable = "as.factor.year.2011")
x[[4]] <- inla.smarginal(m_inla$marginals.fixed$year_factoryear_factor2013) %>%
  as_tibble() %>%
  mutate(variable = "as.factor.year.2013")
x[[5]] <- inla.smarginal(m_inla$marginals.fixed$year_factoryear_factor2015) %>%
  as_tibble() %>%
  mutate(variable = "as.factor.year.2015")
x[[6]] <- inla.smarginal(m_inla$marginals.fixed$year_factoryear_factor2017) %>%
  as_tibble() %>%
