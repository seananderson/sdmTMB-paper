...
abstract: >
  Geostatistical data---spatially referenced observations of a continuous spatial phenomenon---are ubiquitous in ecology and can reveal ecological processes and inform management decisions. However, appropriate models to analyse these data, such as generalised linear mixed effects models (GLMMs) with Gaussian random fields, are computationally intensive and challenging to. Therefore, spatial autocorrelation is frequently ignored, or addressed in less ecologically interpretable ways.
  Here, we introduce the R package sdmTMB, which extends the flexible interface familiar to users of glm(), lme4, glmmTMB, or mgcv to include spatial and spatiotemporal latent fields using a predictive-process SPDE-(stochastic partial differential equation) based approach. Estimation is conducted via maximum marginal likelihood with Template Model Builder (TMB), although penalised likelihood and Bayesian inference (via tmbstan/Stan) are also possible. We describe the statistical model, illustrate the package's use through two case studies, and compare the functionality, speed, and interface to related software.
  The case studies illustrate sdmTMB’s unmatched flexibility and useability by implementing penalised smoothers, non-stationary processes (time-varying and spatially varying relationships), and anisotropy (asymmetry in spatial correlations that commonly occurs along ecological gradients and is not available in INLA/inlabru, the most commonly used packages for this class of models). Modelling of these ecologically ubiquitous, but computationally complex, relationships is further aided by built-in handling of delta/hurdle models, break-point effects, abundance index standardisation, and diagnostic tools including out-of-sample cross validation. Finally, we demonstrate that sdmTMB is 4–16 times faster than INLA or inlabru.
  sdmTMB's accessible interface has already opened this useful class of models to a wider audience within species distribution modelling. We anticipate its application to the full spectrum of problems involving geostatistical data---both within and beyond ecology---will prove equally fruitful.
output:
  bookdown::pdf_book:
...
The SPDE approach is a solution popularized via the R-INLA package [@rue2009; @lindgren2011; @lindgren2015] and an implementation in TMB [Template Model Builder, @kristensen2016] that partially relies on INLA to create input matrices [e.g., @osgood-zimmerman2021].
...
See Article S1 for a full description of the model.
...
By default, if spatiotemporal fields are included, they are assumed IID; however, additional options allow them to be modelled as a random walk or first-order autoregressive, AR(1), process (Fig.\ \@ref(fig:diagram); Article S1).
Turning off both spatial and spatiotemporal effects allows comparison with a standard non-spatial GLM or GLMM.
We include additional flexibility in specifying the linear fixed effect matrix: covariates can be modelled as penalized smooth functions (generalized additive models, GAMs) using the same `s()` syntax as in mgcv [@wood2017a] (thereby allowing automatic selection of smoother 'wiggliness'), or can be modelled with threshold shapes; for example, hockey-stick models, `breakpt()`, [@barrowman2000] or logistic functions, `logistic()` (Article S1).
...
We found a weak average positive effect of the annual mean NAO on overall counts, but a southeast to northwest gradient in the intensity of the effect (Fig. \@ref(fig:owl-nao), Article S3).
...
We provide comparisons of the syntax and the reproducibility of results from models fit using INLA or inlabru (Article S4) and VAST (Article S5).
...
We ran a simple speed comparison between sdmTMB, inlabru/INLA, and mgcv for fitting an SPDE spatial random field model to 1000 data points with Gaussian error across a range of mesh resolutions (Fig. \@ref(fig:timing), Article S6).
...
alternatively, because VAST focuses on multivariate delta models and fisheries applications (Article S5), users fitting "simple" univariate spatial/spatiotemporal GLMMs in non-fisheries contexts may find sdmTMB more straightforward to use.
...
<!-- Furthermore, although both INLA/inlabru and sdmTMB can use parallel processing, the PARDISO library [@pardiso-7.2c] within INLA/inlabru allows memory to be shared across cores. -->
...
Article S1: sdmTMB statistical model description\
Article S2: Example of index standardization of fishery-independent survey data with sdmTMB\
Article S3: Example of a spatially varying effect of climate on citizen science count data with sdmTMB\
Article S4: Species distribution model comparison between INLA and sdmTMB\
Article S5: Comparison of index standardization of survey data with VAST and sdmTMB\
Article S6: sdmTMB speed testing methods and model validation description
