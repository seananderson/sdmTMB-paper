...
author: "|
  Sean C. Anderson$^{1,2}$^[Corresponding author: sean.anderson@dfo-mpo.gc.ca],
  Eric J. Ward$^3$,
  Philina A. English$^1$,
  Lewis A. K. Barnett$^4$\n|
  ...
  $^4$Alaska Fisheries Science Center, National Marine Fisheries Service,\n|
      National Oceanic and Atmospheric Administration, Seattle, WA, USA\n"
output:
...
Geostatistical data---spatially referenced observations of a continuous spatial phenomenon---are ubiquitous in ecology and can reveal ecological processes and inform management decisions. 
However, appropriate models to analyse these data, such as generalised linear mixed effects models (GLMMs) with Gaussian random fields, are computationally intensive and challenging to implement. 
Therefore, spatial autocorrelation is frequently ignored, or addressed in less ecologically interpretable ways.
Here, we introduce the R package sdmTMB, which extends the flexible interface familiar to users of lme4, glmmTMB, or mgcv to include spatial and spatiotemporal latent fields using a predictive-process SPDE-(stochastic partial differential equation) based approach. 
Estimation is conducted via maximum marginal likelihood with Template Model Builder (TMB), although penalised likelihood and Bayesian inference (via tmbstan/Stan) are also possible. 
We describe the statistical model, illustrate the package's use through two case studies, and compare the functionality, speed, and interface to related software.
The case studies illustrate sdmTMB’s unmatched flexibility and usability by implementing penalised smoothers, non-stationary processes (time-varying and spatially varying relationships), and anisotropy (asymmetry in spatial correlations that commonly occurs along ecological gradients and is not available in INLA/inlabru, the most commonly used packages for this class of models). 
Modelling of these ecologically ubiquitous, but computationally complex, relationships is further aided by built-in handling of delta/hurdle models, break-point effects, abundance index standardisation, and diagnostic tools including out-of-sample cross validation. 
Finally, we demonstrate that sdmTMB is 4–16 times faster than INLA or inlabru.
sdmTMB's accessible interface has already opened this useful class of models to a wider audience within species distribution modelling. 
We anticipate that its application to the full spectrum of problems involving geostatistical data---both within and beyond ecology---will prove equally fruitful.
...
<!-- Authors must provide a statement outlining why Ecography is the best outlet for publication. This should include a description of why the submitted work makes a significant contribution to and is of general interest for understanding spatial and temporal patterns in ecology. You must provide a clear statement of how the manuscript builds on both recent work by yourself or your coauthors cited in this manuscript (published during the past 12 months, or in press, or submitted) and other published work. -->
...
<!-- Ecography requires that all authors disclose any potential sources of conflict of interest. Any interest or relationship, financial or otherwise, that might be perceived as influencing an author’s objectivity is considered a potential source of conflict of interest. For more information, please read our ethics and policies page. -->
